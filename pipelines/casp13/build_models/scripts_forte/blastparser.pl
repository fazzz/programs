...
sub blastalign{#blast の結果ファイルからアラインメントを生成する
	...
	my $outfilename = $_[1];
	my $threshold = $_[2];
	my $sourceseq = $_[3];#もし全長が欲しい場合(AA のみ)
...
sub result_to_msa{
	...
	return max_redundant_alignment_nogap(\@qarray,\@qstart,\@sarray);
}


sub max_redundant_alignment_nogap{
	...
	for(my $ii = 0;$ii <= $#qarray;$ii++){
		...
		for(my $jj = $cou;$jj < $endmax;$jj++){
			${$retseq[$ii]}[$jj] = "-";
		}
	}
	
	my @ret;
	push(@ret,join("",@{$retseq[$#retseq]}));
	for(my $jj = 0;$jj <= $#retseq-1;$jj++){
		push(@ret,join("",@{$retseq[$jj]}));
	}
	return \@ret;
}
...
#MULTI-FASTA を与えた（-in_msa でなく）時に複数の Result が出るので、それを一つずつに分ける。
...
#一つのクエリに対する検索結果が入ったファイルを渡すと配列で返す。
