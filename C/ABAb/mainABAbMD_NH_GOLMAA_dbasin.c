...
int main(int argc, char *argv[]) {
  int i,j,k,l=0,d;
  int numatom,numres,numclut,numstep=100000,interval=1000,intervalflag;
  int **nb_matrix;
  double dt=0.001;
  CLTb *clt;
  double *Q,*frc,pot;
  ...
  char *inputfilename,*reffilename1,*reffilename2;
  char *outputfilename,*outputfilename2,*parmfilename,*clustfilename,*inputvelofilename;
  char *trjfilename;
  FILE *inputfile,*reffile1,*reffile2,*inputfilegoal,*outputfile,*outputfile2,*parmfile,*clustfile,*inputvelofile;
  ...
  while((c=getopt_long(argc,argv,"*+h@:t:k:c:t:i:v:|:?:d:V:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case '*':
      MODE=NVE;
      break;
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (TERMMODE==ON)
      ABAb_integ_pret_Term(predict_Term,predict_Term2,correctt_Term,correct_Term2,vel_Term,dt);
    ...
    pot_d=GOLMAA_dbasin_calcTorque(Q,crd,&e,numclut,numclutparent,terminal,origin,delta,deltaV);
    GOLMAAff_dbasin_calcff(crd,numatom,&e,delta,deltaV);
    pot=e.p_t+pot_d;
    ...
    if (TERMMODE==ON)
      ABAb_integ_cort_Term(predict_Term,predict_Term2,correctt_Term,correct_Term2,acc_Term,acc_Term2,vel_Term,dt);
    ...
    if (i%interval==0) {
      fprintf(outputfile,"%d %lf %lf %lf %lf %lf %lf %lf %lf \n",i,pot,KE,KEv,PEv,pot+KE+PEv+KEv,s,T);
      //      GOLMAA_dbasin_out_formated(outputfile2,e,KE,i,dt);
