...
void ABAbp_prepass(CLTb *clt,double *qvel,int numclt,int numatom,double *crd) {
  ...
  num_index_terminal_atom_a=(int)gcemalloc(sizeof(int)*numclt);
  for (j=0;j<numclt;++j) {
    num_index_terminal_atom_a[j]=0;
  }

  ABAbs_trans_Matrix(clt,numclt,numatom,crd);
  ...
  for (i=0;i<numclt;++i) {
    ...
    if (i!=0) {
      j=num_index_terminal_atom_a[nNumClutOfParent];
      ++num_index_terminal_atom_a[nNumClutOfParent];
      ABAb_Coacc(clt[i].Coacc,clt[i].Spvel,clt[nNumClutOfParent].Spvel,qvel[i],
		 clt[i].origin_atom_a,
		 /*clt[nNumClutOfParent].terminal_atom_a[j]*/clt[nNumClutOfParent].origin_atom_a,
		 nNumClutOfParent,
...
void ABAb_Coacc(double Coacc[6],double Spvel[6],double Spvel_P[6],double qvel,
	       int origin_atom_a,int terminal_atom_a_Parent,int nNumminousone,
  ...
  if(nNumminousone==-1) 
    ...
    nNumAtomOfClutminousone = terminal_atom_a_Parent-1;
    for (alpha=0;alpha<3;++alpha)
