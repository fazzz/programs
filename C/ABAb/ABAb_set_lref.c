...
void ABAbs_local_reference(CLTb *clt,int nNumClut_all,int num_atom_all,double *crd) {
  ...
  for (nNumClut=0;nNumClut<nNumClut_all;++nNumClut) {
    ...
    if (nNumClut!=0) {
      i=num_index_terminal_atom_a[nNumClutOfParent];
      ++num_index_terminal_atom_a[nNumClutOfParent];
      nNumAtomP = clt[nNumClutOfParent].terminal_atom_a[i]-1;
      nNumAtomO = clt[nNumClut].origin_atom_a-1;
    }

    if (nNumClut == 0)  {
      clt[nNumClut].xoord=(double *)gcemalloc(sizeof(double)*num_atom_all*3);
      sub_trans_A_to_CN_b(clt[nNumClut].trans_A_to_CN,clt[nNumClut].xoord,
			  0,
	...
	if ((clt[nNumClutdummy].terminal==0) && (clt[nNumClutdummy].join==clt[nNumClut].join))
      ...
      clt[nNumClut].xoord=(double *)gcemalloc(sizeof(double)*num_atom_all*3);
      sub_trans_A_to_CN_b(clt[nNumClut].trans_A_to_CN,
			  clt[nNumClut].xoord,
			  nNumClut, 
			  clt[nNumClut].origin_atom_a,
			  nNumClutOfParent,
			  clt[nNumClutOfParent].terminal_atom_a[i],
			  //			  clt[nNumClutOfParent].origin_atom_a,
			  num,
			  num_atom_all,
			  crd);
