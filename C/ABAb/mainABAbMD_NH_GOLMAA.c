...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt_long(argc,argv,"*+h@:t:k:c:t:i:j:k:v:|:?:{:}:>:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case '*':
      MODE=NVE;
      break;
  ...
  for (i=0;i<numatom;++i) nb_matrix[i]=(int *)gcemalloc(sizeof(int)*numatom); // 2011-11-11
  ...
  if (MODEV==ON) 
    ABAbs_restat_read(inputvelofilename,numclut,correct,correct_Term,correct_Term2,correct_s,MODE,TERMMODE);
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (i%intervalout==0) 
      GOLMAA_out_formated(outputfile2,e,KE,KEv,PEv,i,dt);
  ...
  ABAbs_restat_write_vel(rstvelfilename,numclut,correct,correct_Term,correct_Term2,correct_s,MODE,TERMMODE);
