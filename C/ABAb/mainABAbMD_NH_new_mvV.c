...
int main(int argc, char *argv[]) {
  ...
  ABAbNH_set_new_mvV(s,s_vel,zeta,tau,&tau2,&Q_NH,KEobj,dt);

  if (MODEV==ON) 
    ABAbs_restat_read_new_mvV(inputvelofilename,numclut,qvel_b1,qvel_b2,vel_Term_b1,vel_Term_b2,zeta,MODE,TERMMODE);
  ...
  for (i=0;i<numstep;++i) {
    ...
    for (t=0;t<numite;++t) {
      if (TERMMODE==ON) 
	solverABAb_TermOn_NH_new_mvV(qacc,qvel,clt,Q,frc,crd,numclut,numatom,q,zeta,acc_Term,acc_Term2,vel_Term);
      else solverABAb_NH_new_mvV(qacc,qvel,clt,Q,frc,crd,numclut,numatom,q,zeta);
    ...
    if (MODE==NVT) { 
      ABAb_calcKineE_TermOn(&KE_1_2,&KEv_1_2,&PEv_1_2,KEobj,clt,crd,qvel_b1,s,s_vel,Q_NH,vel_Term_b1,numclut,numatom,MODE,numclut+6);
    ...
    if (MODE==NVT)
      ABAbNH_calcKE_new(zeta,s,s_vel,Q_NH,KEobj,&PEv,&KEv);
    T=KE/(DOF*k_B)*2.0;
  ...
  ABAbs_restat_write_vel_new_mvV(rstvelfilename,numclut,qvel_b1,qvel_b2,vel_Term_b1,vel_Term_b2,zeta,MODE,TERMMODE);
  fclose(outputfile);
