...
int main(int argc, char *argv[]) {
  ...
  ABAb_integ_set_NVT(q_NVT,qvel_NVT,predict_NVT,correct_NVT,dt);
  ...
  for (i=0;i<numstep;++i) {
    ...
    ABAb_integ_pret_NVT(&qvel_NVT,&q_NVT,predict_NVT,correct_NVT,dt);
    ABAb_update(clt,crd,qrot,numclut,numatom);
    if (TERMMDOE==ON) ABAb_update_Term(crd,delta_term,numatom);
    ...
    ABAb_integ_cort_NVT(&qvel_NVT,&q_NVT,qacc_NVT,predict_NVT,correct_NVT,dt);
    ABAb_update(clt,crd,qrot,numclut,numatom);
    if (TERMMDOE==ON) ABAb_update_Term(crd,delta_term,numatom);
