...
void ABAbs_restat_write_vel_new(char *restatvelofilename,int numclut,
  ...
  if (TERMMODE==ON) {
    for (i=0;i<6;++i)  {
      for (j=0;j<6;++j) {
	//	fprintf(restatvelofile,"%10.8lf ",correctt_Term[i][j]);
	fprintf(restatvelofile,"%12.10lf ",correctt_Term[i][j]);
	++l;
	if (l%10==0) fprintf(restatvelofile,"\n");
      }
    }
    for (i=0;i<6;++i)  {
      for (j=0;j<6;++j) {
	//	fprintf(restatvelofile,"%10.8lf ",correctt_Term2[i][j]);
	fprintf(restatvelofile,"%12.10lf ",correctt_Term2[i][j]);
	++l;
	if (l%10==0) fprintf(restatvelofile,"\n");
      }
    }
  ...
  for (i=0;i<numclut;++i) {
    for (j=0;j<6;++j) {
      //      fprintf(restatvelofile,"%10.8lf ",correct[i*6+j]);
      fprintf(restatvelofile,"%12.10lf ",correct[i*6+j]);
      if (l%10==0) fprintf(restatvelofile,"\n");
    }
  }
  ...
  if (MODE==NVT) {
    for (i=0;i<5;++i) {
      //      fprintf(restatvelofile,"%10.8lf\n",correct_gzi[i]);
      fprintf(restatvelofile,"%12.10lf ",correct_gzi[i]);
      if (l%10==0) fprintf(restatvelofile,"\n");
    }
  }
