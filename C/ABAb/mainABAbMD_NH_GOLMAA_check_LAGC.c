...
int main(int argc, char *argv[]) {
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (i>numdb) fprintf(outputfile3,"yes 01\n");
    ...
    if (i>numdb) fprintf(outputfile3,"yes 02\n");
    ...
    if (i>numdb) fprintf(outputfile3,"yes 05\n");
    ...
    if (i>numdb) fprintf(outputfile3,"yes 06\n");

    ABAb_update(clt,crd,qrot,numclut,numatom);
    //    if (TERMMODE==ON) ABAb_update_Term(crd,delta_Term,numatom);
    if (i>numdb) fprintf(outputfile3,"yes 07\n");

    if (TERMMODE==ON)
      ABAb_calcKineE_TermOn(&KE,&KEv,&PEv,KEobj,clt,crd,qvel,s,s_vel,Q_NH,vel_Term,numclut,numatom,MODE,numclut+6);
    else
      ABAb_calcKineE(&KE,&KEv,&PEv,KEobj,clt,crd,qvel,s,s_vel,Q_NH,numclut,numatom,MODE);
    if (i>numdb) fprintf(outputfile3,"yes 08\n");
    if (MODE==NVT)
      ABAbNH_calcKE(s,s_vel,Q_NH,KEobj,&PEv,&KEv);
    if (i>numdb) fprintf(outputfile3,"yes 09\n");
    ...
    if (i>numdb) fprintf(outputfile3,"yes 10\n");
    ...
    if (i>numdb) fprintf(outputfile3,"yes 11\n");
