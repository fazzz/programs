...
void ABAbs_inertia_matrix(CLTb *clt,int nNumClut_all,int num_atom_all,double *crd,double *mass) {
  ...
  /*   joinflag=ABAb_setJoin(clt,nNumClut,joinflag);	  */
  /* }							  */
  /********************************************************/

  for(nNumClut=0; nNumClut<nNumClut_all; ++nNumClut) {
    clt[nNumClut].mass=(double *)gcemalloc(sizeof(double)*clt[nNumClut].num_atom_clust);
    if (nNumClut==0)
      numatomstart=0;
    else
	...
	//	na+=clt[nNumClut+j].num_atom_clust;
      ...
      if (nNumClut==0)
	clt[nNumClut].mass[i]=mass[i+na];
      else
	clt[nNumClut].mass[i]=mass[clt[nNumClut].origin_atom_a-1+i+na];
    }

    ABAbs_Inertia_clust(clt[nNumClut].I,
		       clt[nNumClut].num_atom_clust,
		       clt[nNumClut].xoord,clt[nNumClut].mass);
    ABAbs_InertiaMatrix(clt[nNumClut].IM,
		       clt[nNumClut].I,
...
int ABAb_setJoin(CLTb *clt,int nNumClut) {
  int i,j,k;
  int nNumParent;
