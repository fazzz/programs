...
int main(int argc, char *argv[]) {
  ...
  double *crd,*crdref,*crdref2,vec[3];
  int numatom,numres;
  ...
  char *progname;
  char *inputfilename,*parmtopname,*crdreffilename,*crdreffilename2;
  char *outputfilename,*outputfilename2,outputfilenamepdb[1000];

  FILE *crdfile,*parmtop,*crdreffile,*crdreffile2;
  ...
  while((c=getopt_long(argc,argv,"hdAxnplvk12c:b:N:C:i:a:j:q:g:e:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case 'A':
      AMBERMODEflag=ON;
      break;
  ...
  io_scanconf_Amber_ini(crdreffile2,numatom,crdref2);
  fclose(crdreffile2);
  ...
  GOLMAA_MB_PROTEINS2008_ff_calcff_set(&e_GOLM,crdref,crdref2,
				       numatom,numres,e.parm.indexnb,e.parm.numnb,
				       ep,nibnum,criteria);
  d2=d*d;
  ...
  for (i=0;i<numstep;++i) {
    ...
    if ( Q >= Qmin && Q <= Qmax) {
      GOLMAA_MB_PROTEINS2008_ff_calcff_wobaimp(crd,numatom,de,d2,&e_GOLM);
      fprintf(outputfile2,"%3d p_MB=%8.3e p_1t=%8.3e p_2t=%8.3e\n",i,e_GOLM.p_MB,e_GOLM.e1.p_t,e_GOLM.e2.p_t);
