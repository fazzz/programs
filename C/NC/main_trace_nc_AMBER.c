...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt(argc,argv,"HhAc:"))!=-1) {
    switch(c) {
    case 'H':
      HMODE=INC;
      break;
  ...
  for (i=0;i<numres;++i)
    ncmap[i]=(int *)gcemalloc(sizeof(int)*numres);
  indexncb=make_native_contact_list(&numnc,crdref,numatom,numres,criteria,ncmap/*,HMODE*/);
