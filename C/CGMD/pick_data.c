...
void mdinputscan(FILE *input) {
  ...
  if (y == 0 || y == 1 || y  == 2/*0911*/)	{
    ...
    printf("error; VELOCITY SET KEY MUST BE 0 OR 1 OR 2 !!! \n");
    exit(1);
  }
...
void pick_initial_velode(void) {
  ...
  if (TermMoveMode2==12) {
    ...
    for (i=0;i<prot.DOF;++i) {
      for (j=0;j<6;++j) {
	fscanf(input,"%lf",&clust[i].correct_dihedang[j]);
      }
    }
    if (GearMODE==ON) {
      for (j=0;j<6;++j) {
	fscanf(input,"%lf",&s_NVT_correct[j]);
      }
    }
  }
}
