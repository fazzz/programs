...
int check_conv(int i,double delta);

// インプットファイルの読み込み、シミュレーションの実行、
// 計算結果の出力を行うメイン関数
int main(int argc, char *argv[]) {
  ...
  double predict_Term[6][6],correct_Term[6][6],delta_acc_Term[6],delta;
...
//	MM = 6;
  ...
  // 座標ファイル、トポロジーファイル、計算条件ファイルの読み込み
  //  printf("yes\n");
  pick_data(pflag);
  //  printf("yes\n");
  joinflag=0;
  ...
  // 運動エネルギー、ポテンシャルエネルギー、全エネルギーの初期化
...
//	dot_s_NVT = 0.0;
  ...
  if (GearMODE == OFF) {
	...
	for (j=0;j<6;++j) {
	  ...
	  acc_Term_bh[j]=0.0;
	  //0911          vel_Term[j]=0.0;
	  //0911	  vel_Term_bh[j]=0.0;
	  //0911	  vel_Term_boh[j]=0.0;
	  delta_Term[j]=0.0;
	}
	if (MODEV == ON){
	  for (j=0;j<6;++j) {
	  ...
	  for (j=0;j<6;++j) {
	    vel_Term_bh[j]=vel_Term[j]; 
	    vel_Term_boh[j]=vel_Term[j];
	  }
	}
      }
    }
  ...
  //  printf("yes1-3\n");
	...
	if (MODEV == 2){           //0911
	  // 初期速度の計算        //0911
	  pick_initial_velode();    //0911
	  if ((input=fopen(/*"velo.in"*/InpfilDVELO,"r")) == NULL) { //0911
	    printf("error:cannot open velo.in \n"); //0911
	    exit(1); //0911
	  } //0911
	  if (TermMoveMode2==12) { //0911
	    for (i=0;i<6;++i) {   //0911
	      for (j=0;j<6;++j) {   //0911
		fscanf(input,"%lf",&correct_Term[i][j]); //0911
	      }
	    }                        //0911
	  }                          //0911
	  fclose(input);             //0911
...
///////////////////////////////////////////////////////////////////////////////
	  ...
	  if (GearMODE == ON ) {
	    for(nNumClut=/*1*//*0*/0; nNumClut<prot.DOF; ++nNumClut) {
	      if (clust[nNumClut].num_branch > 1) {
		nNumClutOrigBranch = nNumClut;
	      }
	      trans_CN_to_A(nNumClut, nNumClutOrigBranch, q_Term,delta_Term);
	    }
	  }
	  //  printf("yes4\n");
...
///////////////////////////////////////////////////////////////////////////////
	...
	fclose(rst_v);
        ...
        //////////////////////////////////////////// 0911
	if ((rst_v = fopen("velode.in"/*OutfilRESTAV*/,"w")) == NULL) { 
	  printf("cnt open velode.in\n");                               
	  exit(1);                                                    
	}
	if (TermMoveMode2==12) {
	  for (i=0;i<6;++i) {
	    for (j=0;j<6;++j) {
	      fprintf(rst_v,"%12.8lf ",correct_Term[i][j]);
	    }
	    fprintf(rst_v,"\n ");
	  }
	  for (i=0;i<prot.DOF;++i) {
	    for (j=0;j<6;++j) {
	      fprintf(rst_v,"%12.8lf ",clust[i].correct_dihedang[j]);
	    }
	    fprintf(rst_v,"\n ");
	  }
	  if (GearMODE==ON) {
	    for (j=0;j<6;++j) {
	      fprintf(rst_v,"%12.8lf ",s_NVT_correct[j]);
	    }
	    fprintf(rst_v,"\n ");
	  }
	}
...
void pre_dyn(void) {
  ...
  // sptial velocity、corioli 加速度、corioli 力の計算
  for(nNumClut=0; nNumClut<prot.DOF; ++nNumClut){
    if (clust[nNumClut].num_branch > 1) {
      nNumClutOrigBranch2 = nNumClut;
    }
