...
int InvMat_spacc(double mat[6][6], double inv[6][6]);

void sub_calc_sp_acc_cycle_multimer(int nNumClut, int nNumCltminusone);
...
//	{
  ...
  if (ABI[nNumClut].hingmat!=6) {
    ...
    sub_calc_sp_acc_cycle_multimer(nNumClut, nNumClutOfParent);
  }
//	}
//	else
//	{
...
void sub_calc_sp_acc_cycle_multimer(int nNumClut, int nNumCltminusone) {
  ...
  double temp0[6][6],inv[6][6],add[6];
...
//	}
  ...
  // 剛体の2面角加速度の初期化を行う
  for (i=0;i<6;++i) {
    clust[nNumClut].dddihedang_six[i] = 0.0;
    clust[nNumClut].dddihedang_six2[i] = 0.0;
  }

  for (i=0;i<6;++i)
    for (j=0;j<6;++j)
      temp0[i][j]=ABI[nNumClut].CorrectABI[i][j];
  InvMat_spacc(temp0,inv);

  for(i=0;i<6;++i) {
    for(j=0;j<6;++j) {
      clust[nNumClut].dddihedang_six2[i]-=
	inv[i][j]*(zzz[nNumClut].Correctzzz[j]);
    }
  }
  ...
  add[1]=0.0;
  add[2]=0.0;
  add[3]=(-clust[nNumClut].ddihedang_six[2]*clust[nNumClut].ddihedang_six[3+1]+clust[nNumClut].ddihedang_six[1]*clust[nNumClut].ddihedang_six[3+2]);
  add[4]=( clust[nNumClut].ddihedang_six[2]*clust[nNumClut].ddihedang_six[3+0]-clust[nNumClut].ddihedang_six[0]*clust[nNumClut].ddihedang_six[3+2]);
  add[5]=(-clust[nNumClut].ddihedang_six[1]*clust[nNumClut].ddihedang_six[3+0]+clust[nNumClut].ddihedang_six[0]*clust[nNumClut].ddihedang_six[3+1]);

  for(i=0;i<6;++i) {
    clust[nNumClut].dddihedang_six[i]=clust[nNumClut].dddihedang_six2[i]-add[i];
  }

  if (nNumClut!=0)
    clust[nNumClut].dddihedang_six[i]=clust[nNumClut].dddihedang_six2[i];


////////////////////////////////////////////////////////////////////////
	// 剛体の2面角加速度の計算を行う_1
  for(i=0; i<6; ++i) {
    clust[nNumClut].dddihedang_six[i]/*rad/s^2*/
      -=   clust[nNumClut].predict_alpha[i]/*rad/s^2*/;
  }
...
//	}
  ...
  for (i=0;i<6;++i)
    clust[nNumClut].sp_acc[i] += clust[nNumClut].dddihedang_six2[i];
