...
double Calc_L_J_PotentialandForce(void)
{
	...
	FPIeata_o = 4.0*PI*eata/*_o*//*/1.602177e-19/1.602177e-19*//1.0e-24*1.660539e-27/1.0e-10;/*F^-1*m=m^3*kg*s^-4*A^-2*/
	...
	for(nNumClut = 0;nNumClut < prot.DOF; ++nNumClut)
	{
		for(i=0;i < clust[nNumClut].num_atom_clust;++i)
		{
			...
			for(alpha=0; alpha<3; ++alpha)
			{
				...
				clust[nNumClut].f_c.f_1_4_L_J[i][alpha]/*N*/ = 0.0;
				clust[nNumClut].f_c.f_elesta[i][alpha]/*N*/ = 0.0;
				clust[nNumClut].f_c.f_1_4_elesta[i][alpha]/*N*/ = 0.0;
			}
			++num_a_prot;
		}
	}
...
//	double elect_pote = 0.0;
	double elect_pote2 = 0.0;

	if(len_q[num_a_prot][NUM_A_PROT]>0)
	{
		elect_pote[num_a_prot][NUM_A_PROT] /*J*/ =
					      clust[nNumClut].f_p_clust.e_f[i_c]/*e*/
...
//						  1.0
	...
	{
		elect_pote[num_a_prot][NUM_A_PROT] = 0.0; /*J*/
		printf("error: length of %d th atom and %d th atom is not reasnoable\n", 
                num_a_prot, NUM_A_PROT);
	}

	elect_pote[num_a_prot][NUM_A_PROT]= elect_pote[num_a_prot][NUM_A_PROT]/**2.3889e-4/*Kcal/J*///
	  //						   				  /4.184000/1000.0
	  //					   				  *6.02214e23/*/mol*/
//						   				  *1.11268e-13
						   				  ;
	elect_pote2 = elect_pote[num_a_prot][NUM_A_PROT];
...
//						  1.0
	...
	elect_pote[num_a_prot][NUM_A_PROT]
	  										= elect_pote[num_a_prot][NUM_A_PROT]/**2.3889e-4/*Kcal/J*///
	  //				   				  /4.184000/1000.0
	  //					   				  *6.02214e23/*/mol*/
//						   				  *1.11268e-13
										  ;

	elect_pote2 = elect_pote[num_a_prot][NUM_A_PROT]/1.2;
//	out = fopen("testtest.out","a");
//	fprintf(out, "%d %d %lf %lf \n", num_a_prot+1, NUM_A_PROT+1, elect_pote, 1.0/(len_q[num_a_prot][NUM_A_PROT]*len_q[num_a_prot][NUM_A_PROT]));
//	fclose(out);
...
//		/*-*/-= /*-(7_9)*/
//				     ( clust[nNumClut].f_p_clust.e_f[i_c]
//				     *
//				     1.602177e-19/18.2223/*e*//**1.602e-19*/
//				     *clust[nNumClut2].f_p_clust.e_f[ii_c]
//				     *1.602177e-19/18.2223/*e*//**1.602e-19*/ 
//				     )
//				     /FPIeata_o2/*m^3*kg*s^-4*A^-2*/
////					 1.0
...
//					 )
//				     *1.660539e27*1.0e30*1.0e-24
//				     /**10e-2*/;
		clust[nNumClut].f_c.f_elesta[i_c][alpha]/*N*/
		-=			elect_pote[num_a_prot][NUM_A_PROT]*4.184070*100.0
		  //								  /6.0220e23/*mol*/
		  //						  *4.184070*1000.0
		  //			              ///2.3889e-4/*Kcal/J*//**1.0e-150;*/
		  //		                  /1.6606e-27/1.0e-20*1.0e-24/**1.0e120*/
							*q[num_a_prot][NUM_A_PROT][alpha]
							 /len_q[num_a_prot][NUM_A_PROT]
							 /len_q[num_a_prot][NUM_A_PROT];
	}
}
...
void Calc_stat_elect_f_1_4(int nNumClut, int nNumClut2, int i_c, int ii_c)
{
	...
	Th_len_q = pow/*er*/(len_q[num_a_prot][NUM_A_PROT],3);
...
//							)
//						  /FPIeata_o2/*m^3*kg*s^-4*A^-2*/
//					      /(Th_len_q*1.0e-30)/*m^3*/
//				          *(q[num_a_prot][NUM_A_PROT][alpha]*1.0e-10/*A*//**1.0e-10*//*m/A*/)/**10e-2*/
//				          *1.660539e27*1.0e10*1.0e-24
//					      /1.2;
		clust[nNumClut].f_c.f_1_4_elesta[i_c][alpha]/*N*/
		-=			elect_pote[num_a_prot][NUM_A_PROT]*4.184070*100.0
		  //								  /6.022e23/*mol*/
		  ...
		  //		                  /1.6606e-27/1.0e-20*1.0e-24/**1.0e120*/
							*q[num_a_prot][NUM_A_PROT][alpha]
							 /len_q[num_a_prot][NUM_A_PROT]
							 /len_q[num_a_prot][NUM_A_PROT]/1.2;
...
//	B_i_j = prot.L_J_parm.B[index]/6.022142e23/*mol*/*4.18e3/*J/kcal*/*1.0e-60;
	...
	A_i_j = prot.L_J_parm.A[index]*4.184070*100.0
	  //	  /6.0220e23/*mol*/
	  //								  *4.184070*1000.0
	  //					              ///2.3889e-4/*Kcal/J*//**1.0e-150;*/
	  //				                  /1.6606e-27/1.0e-20*1.0e-24/**1.0e120*/
	  ;
	B_i_j = prot.L_J_parm.B[index]*4.184070*100.0
	  //	  /6.0220e23/*mol*/
	  //							  *4.184070*1000.0
	  //				              ///2.3889e-4/*Kcal/J*//**1.0e-150;*/
	  //			   				  /1.6606e-27/1.0e-20*1.0e-24/**1.0e60*/
	  ;
