...
C...Translated by FPP 6.0 (3.06G3) 02/26/96  10:10:23   -dc
C COMMAND.CMN
C
...
C
      IF(NOUTE .GE. LOUTE) THEN
        IF(IENGY .EQ. 1) THEN
      WRITE (IOUT74) NTR, ACP, F
        END IF
        IF(NTR .EQ. (NTR/100)*100) THEN
      WRITE (IOUT02, '(I8,F8.3,3X,6D13.5)') NTR, ACP, F
      WRITE (IOUT06, '(I8,F8.3,3X,6D13.5)') NTR, ACP, F
        END IF
        NOUTE = 0
      END IF
C
...
CDIR@       IVDEP
            DO J = 1, NUMVAR
               R1X = EIGVAL(J)
               EIGVEC(I,J) = SIG*EIGVEC(I,J)/R1X
               EIGVEC(1+I,J) = SIG*EIGVEC(1+I,J)/R1X
               EIGVEC(2+I,J) = SIG*EIGVEC(2+I,J)/R1X
               EIGVEC(3+I,J) = SIG*EIGVEC(3+I,J)/R1X
               EIGVEC(4+I,J) = SIG*EIGVEC(4+I,J)/R1X
               EIGVEC(5+I,J) = SIG*EIGVEC(5+I,J)/R1X
               EIGVEC(6+I,J) = SIG*EIGVEC(6+I,J)/R1X
               EIGVEC(7+I,J) = SIG*EIGVEC(7+I,J)/R1X
            END DO
         END DO
      ENDIF
C-----------------------------------------------------------------------
...
CDIR@       IVDEP
            DO J = 1, NUMVAR
               R2X = W(J)
               VART(I) = VART(I) + (DBLE(R2X) - 0.5D0)*EIGVEC(I,J)
               VART(1+I) = VART(1+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(1+I,J)
               VART(2+I) = VART(2+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(2+I,J)
               VART(3+I) = VART(3+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(3+I,J)
               VART(4+I) = VART(4+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(4+I,J)
               VART(5+I) = VART(5+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(5+I,J)
               VART(6+I) = VART(6+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(6+I,J)
               VART(7+I) = VART(7+I) + (DBLE(R2X) - 0.5D0)*EIGVEC(7+I,J)
            END DO
         END DO
      ENDIF
CDEBUG
