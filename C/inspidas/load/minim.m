...
C...Translated by FPP 6.0 (3.06G3) 02/26/96  10:08:33   -dc
C COMMAND.CMN
C
...
C
      ...
      DO  42  M = MI, MF
        ...
        ENDIF
        IF (M2.LT.M1)  GOTO  42
      IF (KFIN + 1 - M1.LE.0 .OR. KFIN+1-M1.GE.M2-M1+1) THEN
         IF (M2 - M1 + 1 .GT. 0) THEN
CDIR@       IVDEP
            DO 40 J = 1, M2 - M1 + 1
               JPAIR1(KFIN+J) = JPAIR1(M1+J-1)
               JPAIR2(KFIN+J) = JPAIR2(M1+J-1)
   40       CONTINUE
            KFIN = M2 - M1 + 1 + KFIN
         ENDIF
      ELSE
...
C...Translated by FPP 6.0 (3.06G3) 02/26/96  10:08:33   -dc
      ...
      REAL SUM1U(4090),SUM2U(4090)
      REAL QQQ
      TASK COMMON/Z1FPP1CM/ QQQ(8191)
      EQUIVALENCE (QQQ(1),SUM1U),(QQQ(1),SUM2U)
      DIMENSION A(KA,N),U(KU,N),V(KV,*),Q(N),W(N)
      REAL F1U(MAX0(N,1)),F2U(MAX0(M,1))
      INTEGER J1X
      REAL R1X, R2X, R3X, R4X
      IND=30000
      MN=MIN0(M,N)
...
CDIR@ IVDEP
   ...
   30 Q(I)=G
      ...
      DO J1S = 0, N - IP1, 4089
         J2S = MIN0@(N - IP1 + 1 - J1S,4089)
CDIR@    IVDEP
         DO J = 1, J2S
            SUM1U(J) = 0.
CDIR@       IVDEP
            DO K = 1, M - I + 1
               SUM1U(J) = U(I+K-1,I)*U(I+K-1,IP1+J1S+J-1) + SUM1U(J)
            END DO
            F1U(J) = SUM1U(J)/H
         END DO
      END DO
      CALL SGERX@(M-I+1,N-IP1+1,1.,U(I,I),1,F1U,1,U(I,IP1),1,KU)
   60 G=U(I,IP1)
...
CDIR@ IVDEP
   ...
   70 SUM=U(I,K)*U(I,K)+SUM
      ...
      DO J3S = 0, M - IP1, 4089
         J4S = MIN0@(M - IP1 + 1 - J3S,4089)
CDIR@    IVDEP
         DO J = 1, J4S
            SUM2U(J) = 0.
CDIR@       IVDEP
            DO K = 1, N - IP1 + 1
               SUM2U(J) = U(I,IP1+K-1)*U(IP1+J3S+J-1,IP1+K-1) + SUM2U(J)
            END DO
            F2U(J) = SUM2U(J)/H
         END DO
      END DO
      CALL SGERX@(M-IP1+1,N-IP1+1,1.,F2U,1,U(I,IP1),KU,U(IP1,IP1),1,KU)
...
C      CHARACTER IDENT*8/' '/,ATTRIB*8/' '/
      CHARACTER IDENT*8,ATTRIB*8
      CHARACTER RECS*800
C
CCCC CHANGED BY SUNADA 1995.06.16 CCCCCCCC
      IDENT=' '
      ATTRIB=' '
CCCC CHANGED BY SUNADA 1995.06.16 CCCCCCCC
      IDENT=' '
...
C       CHARACTER*8 CKEY,MNCH,KEYKHI
        CHARACTER INFORM*32
        CHARACTER IDATE*7,MDATE*9,IKM*1
CMSP    CHARACTER IDATE*6,MDATE*8,IKM*1
C        CHARACTER*8 METHOD,ECEPP/'ECEPP'/
        CHARACTER*8 METHOD,ECEPP
C        CHARACTER*8 MODIFY/'MODIFIED'/
        CHARACTER*8 MODIFY
        CHARACTER*72 PROTID
...
CZEUS        CALL FOUTGR(IOUT06, 70,20,IGRPH - IGRPH0 - 1,GX,GY)
CZEUS        CALL FOUTGR(IOUT02,100,30,IGRPH - IGRPH0 - 1,GX,GY)
        IGRPH0 = IGRPH - 1
      END IF
C
C--------- ITERATION BEGINS ------------------------------------------
...
C...Translated by FPP 6.0 (3.06G3) 02/26/96  10:08:33   -dc
      INTEGER ISMAX@
      INTEGER J1S,J2S,J3S,J4S,J5S
      REAL TC1U(8186),TST1U(8186)
      REAL QQQ
      TASK COMMON/Z1FPP1CM/ QQQ(8191)
      EQUIVALENCE (QQQ(1),TC1U),(QQQ(1),TST1U)
...
C
  10  THETA = 0.D0
      IF (N - 1 .GE. 10) THEN
         DO J1S = 0, N - 2, 8185
            J2S = MIN0@(N - 1 - J1S,8185)
CDIR@       IVDEP
            DO 12 I = 1, J2S
               TC1U(I) = DABS(AD(LD(J1S+I)+1))
   12       CONTINUE
            DO I = 1, J2S
               THETA = DMAX1(THETA,TC1U(I))
            END DO
         END DO
      ELSE
CDIR@    NEXTSCALAR
...
C
      ...
      DM = DABS(CDIAG(J))
      DO J4S = 0, N - J - 1, 8185
         J5S = MIN0@(N - J - J4S,8185)
CDIR@    IVDEP
         DO 101 I = 1, J5S
            TST1U(I) = DABS(CDIAG(J+J4S+I))
  101    CONTINUE
         J3S = ISMAX@ (J5S, TST1U, 1)
         IF (TST1U(J3S) .GT. DM) THEN
            DM = TST1U(J3S)
            JQ = J3S + J4S + J
         ENDIF
      END DO
...
CDIR@ IVDEP
  ...
  103 CONTINUE
      ...
      IND2 = INDJQ-JQ
*VOPTION VEC
      IF (IND1 - IND2.LE.0 .OR. IND1-IND2.GE.J-1) THEN
CDIR@    IVDEP
         DO 104 IS = 1, J - 1
            AD(IND1+IS) = AD(IND2+IS)
  104    CONTINUE
      ELSE
CDIR@    NEXTSCALAR
         DO IS = 1, J - 1
...
C
CDIR@ IVDEP
      DO 10 MB = 1, IATMF - IATMI + 1
         RI2 = ONE/((CO(1,MA)-CO(1,IATMI+MB-1))**2+(CO(2,MA)-CO(2,IATMI+
     1      MB-1))**2+(CO(3,MA)-CO(3,IATMI+MB-1))**2)
         RI(IATMI+MB-1) = DSQRT(RI2)
         K = NBTYPE(IATMI+MB-1)*2
         RO2 = PNB(NBA,K)*RI2
         IF (PNB(NBA,K-1) .GE. ZERO) THEN
            RO6 = RO2*RO2*RO2
            ENB00(IATMI+MB-1) = PNB(NBA,K-1)*RO6*(HALF*RO6 - TWO)
         ELSE
            RO4 = RO2*RO2
            RO6 = RO2*RO4
            ENB00(IATMI+MB-1) = -PNB(NBA,K-1)*RO6*(RO6 - TWO*RO4)
         ENDIF
         EES = EES + QA*CHARGE(IATMI+MB-1)*RI(IATMI+MB-1)
         ENB = ENB + ENB00(IATMI+MB-1)
   10 CONTINUE
C
  20    KEY = INTATM(3,L)
        IF (KEY.EQ.0)  GOTO  40
...
C
CDIR@ IVDEP
      DO 30 MB = 1, IATMF - IATMI + 1
         DUM001 = (CO(1,MA)-CO(1,IATMI+MB-1))**2 + (CO(2,MA)-CO(2,IATMI+
     1      MB-1))**2 + (CO(3,MA)-CO(3,IATMI+MB-1))**2
         RI2 = ONE/((CO(1,MA)-CO(1,IATMI+MB-1))**2+(CO(2,MA)-CO(2,IATMI+
     1      MB-1))**2+(CO(3,MA)-CO(3,IATMI+MB-1))**2)
         RI(IATMI+MB-1) = DSQRT(RI2)
         ...
         IF (PNB(NBA,K-1) .GE. ZERO) THEN
            RO6 = RO2*RO2*RO2
            ENB00(IATMI+MB-1) = PNB(NBA,K-1)*RO6*(RO6 - TWO)
         ELSE
            RO4 = RO2*RO2
            RO6 = RO2*RO4
            ENB00(IATMI+MB-1) = -PNB(NBA,K-1)*RO6*(RO6 - TWO*RO4)
...
CMSP  INCLUDE (SIZE)
C...Translated by FPP 6.0 (3.06G3) 02/26/96  10:08:33   -dc
      INTEGER J1S,J2S,JPA1U(2042),JPB1U(2042),II1U(2042)
      LOGICAL L1V(2042)
      REAL QQQ
      TASK COMMON/Z1FPP1CM/ QQQ(8191)
      EQUIVALENCE (QQQ(1),JPA1U),(QQQ(2043),JPB1U),(QQQ(4085),II1U),(QQQ
     .(6127),L1V)
C SIZE
C  THIS COMMON BLOCK STORES THE NUMBERS RELATED TO PROTEIN SIZE
C
...
C
      II = 0
      IF (LFIN .GE. 10) THEN
         DO J1S = 0, LFIN - 1, 2041
            J2S = MIN0@(LFIN - J1S,2041)
CDIR@       IVDEP
            DO 10 L = 1, J2S
               JPA1U(L) = IABS@(IPAIR1(J1S+L))
               JPB1U(L) = IPAIR2(J1S+L)
               R2 = (CO(1,JPA1U(L))-CO(1,JPB1U(L)))**2 + (CO(2,JPA1U(L))
     1            -CO(2,JPB1U(L)))**2 + (CO(3,JPA1U(L))-CO(3,JPB1U(L)))
     2            **2
               RI2 = ONE/R2
               EES = EES + CHARGE(JPA1U(L))*CHARGE(JPB1U(L))*DSQRT(RI2)
               IPB(J1S+L) = (NBTYPE(JPB1U(L))-1)*56 + NBTYPE(JPA1U(L))
               RO2(J1S+L) = PNB(IPB(J1S+L)+28)*RI2
               R(J1S+L) = DSQRT(R2)
               L1V(L) = NFLAG(JPA1U(L)).EQ.1 .AND. NFLAG(JPB1U(L)).EQ.1
   10       CONTINUE
            DO L = 1, J2S
               IF (L1V(L)) THEN
                  II = II + 1
...
CDIR@       IVDEP
            DO L = 1, J2S
               IF (L1V(L)) THEN
                  MFLAGX(II1U(L)) = L + J1S
                  R2X(II1U(L)) = (COX(1,JPA1U(L))-COX(1,JPB1U(L)))**2 + 
     1               (COX(2,JPA1U(L))-COX(2,JPB1U(L)))**2 + (COX(3,
     2               JPA1U(L))-COX(3,JPB1U(L)))**2
               ENDIF
            END DO
         END DO
      ELSE
CDIR@    NEXTSCALAR
     ...
     1         CO(3,JPA)-CO(3,JPB))**2
            ...
            IF (NFLAG(JPA).EQ.1 .AND. NFLAG(JPB).EQ.1) THEN
               II = II + 1
               MFLAGX(II) = L
               R2X(II) = (COX(1,JPA)-COX(1,JPB))**2 + (COX(2,JPA)-COX(2,
     1            JPB))**2 + (COX(3,JPA)-COX(3,JPB))**2
            ENDIF
         END DO
      ENDIF
C
...
CMSP  INCLUDE (SIZE)
C...Translated by FPP 6.0 (3.06G3) 02/26/96  10:08:33   -dc
      INTEGER J1S,J2S,J3S,J4S,JPA1U(2042),JPB1U(2042),II1U(2042),L1U(409
     .0)
      REAL AR1U(4090)
      LOGICAL L1V(2042)
      REAL QQQ
      TASK COMMON/Z1FPP1CM/ QQQ(8191)
      EQUIVALENCE (QQQ(1),JPA1U),(QQQ(2043),JPB1U),(QQQ(4085),II1U),(QQQ
     .(6127),L1V),(QQQ(1),L1U),(QQQ(4091),AR1U)
C SIZE
C  THIS COMMON BLOCK STORES THE NUMBERS RELATED TO PROTEIN SIZE
C
      COMMON/SIZE/NUMATM,NUMVAR,NN2,NUMINT,NSS,NUMRES
...
CDIR@       IVDEP
            DO 10 L = 1, J2S
               JPA1U(L) = IABS@(IPAIR1(J1S+L))
               JPB1U(L) = IPAIR2(J1S+L)
               XA(J1S+L) = CO(1,JPA1U(L))
               YA(J1S+L) = CO(2,JPA1U(L))
               ZA(J1S+L) = CO(3,JPA1U(L))
               XB(J1S+L) = CO(1,JPB1U(L))
               YB(J1S+L) = CO(2,JPB1U(L))
               ZB(J1S+L) = CO(3,JPB1U(L))
               XAB(J1S+L) = XA(J1S+L) - XB(J1S+L)
               YAB(J1S+L) = YA(J1S+L) - YB(J1S+L)
               ZAB(J1S+L) = ZA(J1S+L) - ZB(J1S+L)
               R2 = XAB(J1S+L)*XAB(J1S+L) + YAB(J1S+L)*YAB(J1S+L) + ZAB(
     1            J1S+L)*ZAB(J1S+L)
               R(J1S+L) = DSQRT(R2)
               RI2(J1S+L) = ONE/R2
               Q = CHARGE(JPA1U(L))*CHARGE(JPB1U(L))/R(J1S+L)
               EES = EES + Q
               CAB(J1S+L) = -Q*RI2(J1S+L)
               DAB(J1S+L) = THREE*(Q*RI2(J1S+L)*RI2(J1S+L))
               IPB(J1S+L) = (NBTYPE(JPB1U(L))-1)*56 + NBTYPE(JPA1U(L))
               RO2(J1S+L) = PNB(IPB(J1S+L)+28)*RI2(J1S+L)
               L1V(L) = NFLAG(JPA1U(L)).EQ.1 .AND. NFLAG(JPB1U(L)).EQ.1
   10       CONTINUE
            DO L = 1, J2S
               IF (L1V(L)) THEN
...
CDIR@       IVDEP
            DO L = 1, J2S
               IF (L1V(L)) THEN
                  MFLAGX(II1U(L)) = L + J1S
                  R2XXX = (COX(1,JPA1U(L))-COX(1,JPB1U(L)))**2 + (COX(2,
     1               JPA1U(L))-COX(2,JPB1U(L)))**2 + (COX(3,JPA1U(L))-
     2               COX(3,JPB1U(L)))**2
                  RX(II1U(L)) = DSQRT(R2XXX)
                  WGHT(II1U(L)) = ONE/R2XXX*WVLTN
               ENDIF
            END DO
         END DO
      ELSE
CDIR@    NEXTSCALAR
         DO L = 1, LFIN
            ...
            IF (NFLAG(JPA).EQ.1 .AND. NFLAG(JPB).EQ.1) THEN
               II = II + 1
               MFLAGX(II) = L
               R2XXX = (COX(1,JPA)-COX(1,JPB))**2 + (COX(2,JPA)-COX(2,
     1            JPB))**2 + (COX(3,JPA)-COX(3,JPB))**2
               RX(II) = DSQRT(R2XXX)
...
C
      IF (II .GE. 10) THEN
         DO J3S = 0, II - 1, 4089
            J4S = MIN0@(II - J3S,4089)
CDIR@       IVDEP
            DO 12 I = 1, J4S
               L1U(I) = MFLAGX(J3S+I)
               ERRR = ERRR + WGHT(J3S+I)*(R(L1U(I))-RX(J3S+I))**2
               AR1U(I) = RX(J3S+I)/R(L1U(I))
   12       CONTINUE
            DO I = 1, J4S
               CAB(L1U(I)) = CAB(L1U(I)) + TWO*WGHT(J3S+I)*(ONE - 
     1            AR1U(I))
            END DO
            DO I = 1, J4S
               DAB(L1U(I)) = DAB(L1U(I)) + TWO*WGHT(J3S+I)*AR1U(I)*RI2(
     1            L1U(I))
            END DO
         END DO
      ELSE
