...
int main(int argc, char *argv[])
{
	...
	double qz[2][3];
	double test3[10000];
	double psi1ans[100],psi2ans[100],phi1ans[100],phi2ans[100];
...
//	get_initial_values(pep1, pep2,alpha,beta,rou,eta);
	...
	for (i=0;i<3;++i)
	{
		...
		p1[i] = pep2[2][i]-pep2[0][i];
		p2[i] = pep1[1][i]-pep1[0][i];
	}
...
//	eta = 1.242;
		...
		if (test1 == 1 && test2 == 1)
		{
			...
			for (j=0;j<2;++j)
			{
				for (k=0;k<2;++k)
				{
					...
					{
						psi1ans[m] = psi1*180.0/pi;
						psi2ans[m] = acos(cospsi2[j][k])*180.0/pi;
						if (sinpsi2[j][k] < 0.0)
						{
							psi2ans[m] = -psi2ans[m]+360.0;
						}
						phi1ans[m] = acos(cosphi1[j])*180.0/pi;
						if (sinphi1[j] < 0.0)
						{
							phi1ans[m] = -phi1ans[m]+360.0;
						}
						phi2ans[m] = acos(cosphi2[k])*180.0/pi;
						if (sinphi2[k] < 0.0)
						{
							phi2ans[m] = -phi2ans[m]+360.0;
						}
						++m;
					}
					++l;
	...
	{
		if (test3[i] == 1)
			printf("%-5lf %5.3lf %5.3lf %5.3lf %5.3lf\n",(double)i*pi/(180.0*NUM),g[i][0],g[i][1],g[i][2],g[i][3]);
	}

	printf("\n");
	printf("num   psi1      psi2      phi1     phi2     \n");
	for (i=0;i<numans;++i)
	{
		printf("%-d: %5.1lf %5.1lf %5.1lf %5.1lf\n",i,psi1ans[i],psi2ans[i],phi1ans[i]-180.0,phi2ans[i]-180.0);
	}
	...
	fprintf(out,"num   psi1      psi2      phi1     phi2     \n");
	for (i=0;i<numans;++i)
	{
...
//			if (j%2 == 0)
//			{
//				sinpsi2[i][j] = -sqrt(1.0-cospsi2[i][j]*cospsi2[i][j]);
//			}
//			else
//			{
//				sinpsi2[i][j] = sqrt(1.0-cospsi2[i][j]*cospsi2[i][j]);
