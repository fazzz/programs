...
double calc_u(double u[3], double cospsi, double sinpsi, double cosphi, double sinphi, double cosalpha, double sinalpha, double cosbeta, double sinbeta);
double calc_s(double s[3], double cospsi, double sinpsi, double cosphi, double sinphi, double cosalpha, double sinalpha, double cosbeta, double sinbeta, double q[3]);
...
int main(int argc, char argv[])
{
	...
	{
		psi = argv[1]*pi/180.0;
		phi = argv[2]*pi/180.0+pi;
	}

	calc_u(u, cos(psi), sin(psi), cos(phi), sin(phi), cos(alpha) ,sin(alpha), cos(beta) ,sin(beta));
	calc_s(s, cos(psi), sin(psi), cos(phi), sin(phi), cos(alpha) ,sin(alpha), cos(beta) ,sin(beta) ,q);
	printf("%lf %lf %lf\n",u[0],u[1],u[2]);
	printf("%lf %lf %lf\n",s[0],s[1],s[2]);
...
double calc_u(double u[3], double cospsi, double sinpsi, double cosphi, double sinphi, double cosalpha, double sinalpha, double cosbeta, double sinbeta)
{
	...
	fomRmat(cospsi, sinpsi, Rpsi2);
	fomRmat(cosphi, sinphi, Rphi1);
	fomRmat(cosphi, sinphi, Rphi2);
	...
	vect[0] = cosbeta;
	vect[1] = sinbeta*cospsi;
	vect[2] = sinbeta*sinpsi;
