...
double calc_v(double v[3], double cospsi, double sinpsi, double cosphi, double sinphi,double cospsi2, double sinpsi2, double cosphi2, double sinphi2, double cospsi3, double sinpsi3, double cosphi3, double sinphi3, double cosalpha, double sinalpha, double cosbeta, double sinbeta);
...
{
	double g;
	double psi,phi,psi2,phi2,psi3,phi3;
	double alpha,beta;
	double u[3],s[3],v[3],q[3];
	FILE *in, *out;
...
/*	if (argc <= 1)
	...
	{*/
		...
		fscanf(in,"%lf %lf %lf %lf %lf %lf", &psi, &phi, &psi2, &phi2, &psi3, &phi3);
		psi = psi*pi/180.0;
		phi = phi*pi/180.0+pi;
		psi2 = psi2*pi/180.0;
		phi2 = phi2*pi/180.0+pi;
		psi3 = psi3*pi/180.0;
		phi3 = phi3*pi/180.0+pi;
		fclose(in);
//	}
...
calc_v(v, cos(psi), sin(psi), cos(phi), sin(phi),cos(psi2), sin(psi2), cos(phi2), sin(phi2), cos(psi3), sin(psi3), cos(phi3), sin(phi3), cos(alpha), sin(alpha), cos(beta), sin(beta));
	printf("%lf %lf %lf\n",s[0],s[1],s[2]);
	printf("%lf %lf %lf\n",u[0],u[1],u[2]);
	printf("%lf %lf %lf\n",v[0],v[1],v[2]);
...
{
    double q[3];
	double Talpha[3][3],Tbeta[3][3],Rpsi1[3][3],Rphi1[3][3],Rpsi2[3][3],Rphi2[3][3],Rpsi3[3][3],Rphi3[3][3];
	double TaRs1[3][3],TaRs1Tb[3][3],TaRs1TbRh1[3][3],TaRs1TbRh1Ta[3][3],TaRs1TbRh1TaRs2[3][3],TaRs1TbRh1TaRs2Tb[3][3],TaRs1TbRh1TaRs2TbRh2[3][3],TaRs1TbRh1TaRs2TbRh2Ta[3][3],TaRs1TbRh1TaRs2TbRh2TaRs3[3][3],TaRs1TbRh1TaRs2TbRh2TaRs3Tb[3][3];

	fomTmat(cosalpha, sinalpha, Talpha);
	fomTmat(cosbeta, sinbeta, Tbeta);
	fomRmat(cospsi, sinpsi, Rpsi1);
	fomRmat(cospsi2, sinpsi2, Rpsi2);
	fomRmat(cospsi3, sinpsi3, Rpsi3);
	fomRmat(cosphi, sinphi, Rphi1);
	fomRmat(cosphi2, sinphi2, Rphi2);
	fomRmat(cosphi3, sinphi3, Rphi3);
	...
	mvult(TaRs1TbRh1TaRs2TbRh2TaRs3Tb,q,v);
}
