...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt_long(argc,argv,"has:t:|:>:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case 't':
      Tobj=atof(optarg);
      break;
  ...
  for (i=0;i<numatom;++i)
    nb_matrix[i]=(int *)gcemalloc(sizeof(int)*numatom);
  ...
  if (numsflag==ON) numstep=numstepspe;
  ...
  for (i=0;i<numstep;++i) {
    ...
    GOLMAAff_calcff(crd,numatom,&e,ON,ON,ON,nb_matrix);
    pot=e.p_t;
