...
int main(int argc, char *argv[]) {
  ...
  crdref1=(double *)gcemalloc(sizeof(double)*numCAatom*3);
  crdref2=(double *)gcemalloc(sizeof(double)*numCAatom*3);
  ...
  GOLM_Clementi_MB_ff_set_calcff_wmCutOff(&e_GOLM,crdref1,crdref2,refcrdAA1,refcrdAA2,numCAatom,numatom,ep,criteria);
  ...
  for (i=0;i<numstep;++i) {
    if (AMBERMODEflag==ON) myncL_open_inq_get_sh_AMBER(inputfilename,numCAatom,i,1,i+1,&nc_id,crd_nc);
    else myncL_open_inq_get_sh_MCD(inputfilename,numCAatom,i,1,i+1,&nc_id_MCD,crd_nc);
    for (j=0;j<numCAatom;++j)for (k=0;k<3;++k)crd[j*3+k]=crd_nc[j][k];

    Kai=GOLM_Clementi_MB_Kai(crd,numCAatom,de,d,d2,&e_GOLM);
