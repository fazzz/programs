...
int main(int argc, char *argv[]) {
  ...
  double *crd,*crdref1,*crdref2,vec[3];
  int numatom,numres;
  ...
  io_scanconf_Amber_ini(crdreffile1,numatom,crdref1);
  fclose(crdreffile1);
  ...
  for (i=0;i<numstep;++i) {
    if (crdMODE==OFF) {
      if (AMBERMODEflag==ON) myncL_open_inq_get_sh_AMBER(inputfilename,numatom,i,1,i+1,&nc_id,crd_nc);
    ...
    if (wobaimpflag==OFF)
      ...
      //      Kai=GOLMAA_MB_PROTEINS2008_wobaimp_Kai_debug(crd,numatom,de,d,d2,&e_GOLM);
      Kai=GOLMAA_MB_PROTEINS2008_wobaimp_Kai(crd,numatom,de,d,d2,&e_GOLM);
