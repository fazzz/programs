...
int main(int argc, char *argv[]) {
  ...
  progname=argv[0];
  while((c=getopt(argc,argv,"BADLEchaX:F:O:n:i:j:k:d:"))!=-1) {
    switch(c) {
    case 'h':
      USAGE(progname);
      exit(1);
    case 'X':
      beta_ini=atof(optarg);
      break;
    case 'F':
      beta_fin=atof(optarg);
      break;
    case 'O':
      num_dec_step=atoi(optarg);
      break;
    case 'B':
      flagB=OFF;
      break;
  ...
  dbeta=(beta_fin-beta_ini)/num_dec_temp;
  num_dec_step=numstep/num_dec_temp;
  ...
  beta=beta_ini;
  for (i=0;i<numstep;++i) {
    if ((i%num_dec_step)==0) beta-=dbeta;
