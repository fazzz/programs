...
int main(int argc, char *argv[]) {
  ...
  while((c2=getopt(argc,argv,"d:"))!=-1) {
    ...
    case 'd':
      ...
      break;
    default:
      printf("USAGE: ./CSPE.exe numatom flag(c or v) flag2(c or a or d )  inputfilename1(vel or trj) inputfilename2(cond) inputfilename3(parmtop) outputfilename\n");
  ...
  if (flag2=='d')
    ...
    io_dismisstrj(inputfile1,num_step_ini-1,numres);
  else if (flag2=='a')
  ...
  for (i=0;i<numstep;++i) {
	...
	fprintf(outputfile,"%e %e \n",(double)i/numstep/deltat/c,speave[i]/sumspe*s*2);
    }
    else {
