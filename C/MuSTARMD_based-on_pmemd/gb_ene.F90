...
! Local variables:
  ...
  if (igb .eq. 1 .or. igb .eq. 2 .or. igb .eq. 5 .or. igb .eq. 7) then
...
! Set up the GB/SA data structures if we're doing a GB/SA calculation
  ...
  if (gbsa .eq. 1) then
    ...
    do i = 1, atm_cnt
      ...
      if (atomicnumber .eq. 6) then
        if(total_bonds .eq. 4 .and. .not. &
           (isymbl(1:1).ge.'a' .and. isymbl(1:1).le.'z' .or. &
            isymbl(2:2).ge.'a' .and. isymbl(2:2).le.'z')) then
  ...
  call gpu_final_gb_setup(igb, alpb, saltcon, rgbmax, gb_neckcut, gb_fs_max, &
...
! Local variables:
  ...
  if (onstep) then
    ...
    if (gbsa .eq. 1) call gbsa_lcpo(crd, frc, neibr_cnt, max_i, frespa, esurf)
