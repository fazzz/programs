...
/* Enforce use of CUDA 3.2 or later due to rampant compiler bugs in CUDA 3.0 and absence of curand library until 3.2 */
#if defined(CUDA_VERSION) && (CUDA_VERSION < 3020)
#error "CUDA support requires the use of a 3.2 or later CUDA toolkit. Aborting compilation."
#endif


/* Control of single and double precision use. If neither use_SPSP
   ...
   while defining use_SPSP makes it use single precision throughout 
   with the exception of shake which is always done in double precision.
...
#endif
#elif defined(use_SPSP)
typedef float PMEDouble;
typedef float2 __align(8)  PMEDouble2;
typedef float4 __align(16) PMEDouble4;
typedef float PMEFloat;
typedef float2 __align(8)  PMEFloat2;
typedef float4 __align(16) PMEFloat4;
typedef cufftComplex __align(8) PMEComplex;
typedef unsigned long long int PMEUllInt;
#ifdef MPI
static const MPI_Datatype MPI_PMEFLOAT = MPI_FLOAT;
static const MPI_Datatype MPI_PMEDOUBLE = MPI_FLOAT;
#endif
#else
#ifdef WINDOWS
typedef double PMEDouble;
...
static const int SM_13_RSBUILD_NEIGHBORLIST_THREADS_PER_BLOCK   = 288;
static const int SM_2X_RSBUILD_NEIGHBORLIST_THREADS_PER_BLOCK   = 288;
...
static const int SM_13_SHAKE_THREADS_PER_BLOCK                  = 192;
static const int SM_2X_SHAKE_THREADS_PER_BLOCK                  = 512;
...
#define ESCALE 1000000000.0
#define FSCALE 1000000000000.0
static __constant__ const PMEDouble ENERGYSCALE                  = (PMEDouble)1000000000.0;
static __constant__ const PMEDouble ONEOVERENERGYSCALE           = (PMEDouble)1.0 / ESCALE;
static __constant__ const PMEDouble ONEOVERENERGYSCALESQUARED    = (PMEDouble)1.0 / (ESCALE * ESCALE);
static __constant__ const PMEDouble FORCESCALE                   = (PMEDouble)1000000000000.0;
static __constant__ const PMEDouble ONEOVERFORCESCALE            = (PMEDouble)1.0 / FSCALE;


struct RSNLRecordStruct
{
    unsigned int xl;
    unsigned int yl;
    unsigned int zl;
    ...
    unsigned int maxZ;
    unsigned int cells;
    unsigned int cell[RSNLNEIGHBORCELLS];
};

union RSNLRecord
{
    RSNLRecordStruct NL;
    unsigned int array[RSNLNEIGHBORCELLS + 10];
};

struct RSNLEntryStruct
{
    unsigned int xl;
    unsigned int yl;
    unsigned int zl;
    unsigned atoms;
    unsigned int* pAtomList;
};
...
struct cudaSimulation {
    ...
    PMEDouble                   amd_temp0;             
    PMEDouble*                  pAmdWeightsAndEnergy;
    ...
    unsigned long long int*     pUllForce;                          // Fixed point Atom forces
    unsigned long long int*     pUllForceX;                         // Fixed point Atom X forces
    unsigned long long int*     pUllForceY;                         // Fixed point Atom Y forces
    unsigned long long int*     pUllForceZ;                         // Fixed point Atom Z forces    
#ifdef MPI
    ...
    // PME data
    RSNLRecord*                 pRSNLRecord;                        // PME reicprocal sum neighbor list
    int                         RSNLRecords;                        // Number of reciprocal sum neighbor list records;
    unsigned int*               pRSNLPosition;                      // Position in building reciprocal sum neighbor list    
    int*                        plXYZ_q;                            // PME charge grid cast as long
    PMEFloat*                   pXYZ_q;                             // PME charge grid/charge grid buffer
    ...
    int                         constraintSoluteMolecules;          // Total solute constraint molecules
    int                         constraintSoluteMoleculeStride;     // Total solute constraint molecule stride
    int                         constraintSoluteAtoms;              // Total solute constraint atoms
    int                         constraintSolventMolecules;         // Total solvent constraint molecules
    int*                        pConstraintSoluteAtomMoleculeID;    // List of solute constraint molecule IDs
    int*                        pConstraintSoluteAtomConstraintID;  // List of solute constraint atom IDs
    PMEDouble*                  pConstraintSoluteAtomMass;          // Solute constraint atom masses
    PMEDouble*                  pConstraintSoluteAtomX;             // Solute constraint atom x
    PMEDouble*                  pConstraintSoluteAtomY;             // Solute constraint atom y
    PMEDouble*                  pConstraintSoluteAtomZ;             // Solute constraint atom z
    PMEDouble*                  pConstraintSoluteCOMX;              // X Last centers of mass for each solute constraint molecule
    PMEDouble*                  pConstraintSoluteCOMY;              // Y Last centers of mass for each solute constraint molecule
    PMEDouble*                  pConstraintSoluteCOMZ;              // Z Last centers of mass for each solute constraint molecule
    PMEDouble*                  pConstraintSoluteDeltaCOMX;         // X change in center of mass for each solute constraint molecule
    PMEDouble*                  pConstraintSoluteDeltaCOMY;         // Y change in center of mass for each solute constraint molecule
    PMEDouble*                  pConstraintSoluteDeltaCOMZ;         // Z change in center of mass for each solute constraint molecule    
    PMEUllInt*                  pConstraintSoluteUllCOMX;           // X Current center of mass for each solute constraint molecule
    PMEUllInt*                  pConstraintSoluteUllCOMY;           // Y Current center of mass for each solute constraint molecule
    PMEUllInt*                  pConstraintSoluteUllCOMZ;           // Z Current center of mass for each solute constraint molecule     
    PMEDouble*                  pConstraintSoluteInvMass;           // Total Inverse mass for each solute constraint molecule
    int*                        pConstraintSolventAtoms;            // Solvent constraint molecule atom counts
    int4*                       pConstraintSolventConstraint;       // Solvent constraint molecule per atom constraints
    PMEDouble*                  pConstraintSolventAtomMass1;        // First solvent constraint atom mass
    PMEDouble*                  pConstraintSolventAtomX1;           // First solvent constraint atom x
    PMEDouble*                  pConstraintSolventAtomY1;           // First solvent constraint atom y
    PMEDouble*                  pConstraintSolventAtomZ1;           // First solvent constraint atom z
    PMEDouble*                  pConstraintSolventAtomMass2;        // Second solvent constraint atom mass
    PMEDouble*                  pConstraintSolventAtomX2;           // First solvent constraint atom x
    PMEDouble*                  pConstraintSolventAtomY2;           // First solvent constraint atom y
    PMEDouble*                  pConstraintSolventAtomZ2;           // First solvent constraint atom z
    PMEDouble*                  pConstraintSolventAtomMass3;        // Third solvent constraint atom mass
    PMEDouble*                  pConstraintSolventAtomX3;           // First solvent constraint atom x
    PMEDouble*                  pConstraintSolventAtomY3;           // First solvent constraint atom y
    PMEDouble*                  pConstraintSolventAtomZ3;           // First solvent constraint atom z
    PMEDouble*                  pConstraintSolventAtomMass4;        // Fourth solvent constraint atom mass
    PMEDouble*                  pConstraintSolventAtomX4;           // First solvent constraint atom x
    PMEDouble*                  pConstraintSolventAtomY4;           // First solvent constraint atom y
    PMEDouble*                  pConstraintSolventAtomZ4;           // First solvent constraint atom z
    PMEDouble*                  pConstraintSolventCOMX;             // X Last centers of mass for each solvent constraint molecule
    PMEDouble*                  pConstraintSolventCOMY;             // Y Last centers of mass for each solvent constraint molecule    
    PMEDouble*                  pConstraintSolventCOMZ;             // Z Last centers of mass for each solvent constraint molecule
    PMEDouble*                  pConstraintSolventInvMass;          // Total inverse mass for eache solvent constraint molecule
...
struct _gpuContext {
    ...
    GpuBuffer<unsigned long long>* pbUllForce;              // Unsigned long long forces for NTP runs
#ifdef MPI
    ...
    GpuBuffer<RSNLRecord>*      pbRSNLRecord;               // PME Reciprocal Sum Neighbor list
    GpuBuffer<unsigned int>*    pbRSNLPosition;             // Position in building reciprocal sum neighbor list        
    GpuBuffer<PMEFloat>*        pbXYZ_q;                    // PME charge grid
    ...
    GpuBuffer<int>*             pbConstraintSoluteID;               // Physical constraint and molecule ID
    GpuBuffer<PMEDouble>*       pbConstraintSoluteAtom;             // Solute constraint atom positions and masses
    GpuBuffer<PMEDouble>*       pbConstraintSolute;                 // Current and last centers of mass and inverse mass for each solute constraint molecule
    GpuBuffer<PMEUllInt>*       pbConstraintUllSolute;              // Current COM and in integer form
    GpuBuffer<int>*             pbConstraintSolventAtoms;           // Number of atoms in each solvent constraint
    GpuBuffer<PMEDouble>*       pbConstraintSolventAtom;            // Solvent constraint atom positions and masses, total inverse mass, and COM
    GpuBuffer<int4>*            pbConstraintSolventConstraint;      // List of solvent constraint constraint IDs
    ...
    // Accumulation buffers
    GpuBuffer<PMEDouble>*       pbReffBuffer;               // Effective Born Radius accumulation buffer
    GpuBuffer<PMEDouble>*       pbSumdeijdaBuffer;          // Sumdeijda accumulation buffer
    GpuBuffer<PMEDouble>*       pbForceBuffer;              // Force accumulation buffer
    ...
    // AMD buffers
    GpuBuffer<PMEDouble>*       pbAmdWeightsAndEnergy;       // AMD
    GpuBuffer<PMEDouble>*       pbAMDfwgtd;
    GpuBuffer<PMEUllInt>*       pbAMDEDihedral;
