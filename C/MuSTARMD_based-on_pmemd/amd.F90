...
!AMD DUAL BOOST CALC START
   if(iamd.gt.0)then
     ...
     if (master.and. (num_amd_recs.gt.0)) then
...
!AMD DUAL BOOST CALC START
   if(iamd.gt.0)then
     ...
     tboostall = 0.0d0
     fwgt   = 1.0d0
     totalenergy = tot_potenergy + (tboost*temp0*1.987d0)/1000.0d0
     if (((iamd == 1).or.(iamd == 3)) .and. (totalenergy.le.EthreshP)) then
       if (num_amd_lag .eq. 0) then
       tboostall = ((EthreshP - totalenergy)**2)*1000.0d0/ &
                 ((alphaP + (EthreshP - totalenergy))*temp0*1.987d0)
       fwgt = (alphaP**2)/((alphaP + EthreshP - totalenergy)**2)
#ifdef MPI
...
! Local variables:
  ...
  do i=1,ntwx
    write(amdlog,'(2x,3i10,6f22.12)'),ntwx,total_nstep,istep+i, &
