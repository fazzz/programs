...
int main(int argc, char *argv[]) {
  ...
  char *inputfilename,*outputfilename,*pathfilename,*pmffilename,*KLdfilename;
  FILE *inputfile,*outputfile,*pathfile,*pmffile,*KLdfile;
  ...
  while((c=getopt_long(argc,argv,"hdpo:x:a:y:m:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case 'd':
      flagd=ON;  break;
    case 'x':
