...
int main(int argc, char *argv[]) {
  ...
  if (macminflag==OFF)
    ...
    hist=MBAR_AVE_twod_wmaxmin(fene,enek,n_sim,n,twod_data,width,max,min,frame);
  ...
  for (i=0;i</*=*/frame[0];++i) {
	...
	pmf_min=pmf[i*frame[1]+j];
      if ( pmf_max > pmf[i*frame[1]+j] && pmf[i*frame[1]+j]!=0 ) {
	pmf_max=pmf[i*frame[1]+j];
  ...
  for (i=0;i<frame[0];++i) {
	...
	if ( udflag==OFF )
	  ...
	  if ( i*frame[1]+j!=index_max )
	    fprintf(pmffile,"%10.4lf %10.4lf ******\n",min[0]+width[0]*i,min[1]+width[1]*j/*,pmf_min-pmf_max*/);
	  else
	    fprintf(pmffile,"%10.4lf %10.4lf 0.0\n",min[0]+width[0]*i,min[1]+width[1]*j);
	}
      }
