...
int main(int argc, char *argv[]) {
  ...
  char *progname;
  char *enefilename,*enelistfilename,*fenefilename,*Tsimlistfilename;
  FILE *enefile,*enelistfile,*fenefile,*Tsimlistfile;
  ...
  progname=argv[0];
  while((c=getopt(argc,argv,"hnp:c:x:t:s:"))!=-1) {
    switch(c) {
    case 't':
      T=atof(optarg);
      break;
  ...
  expeneraw=(double **)gcemalloc(sizeof(double *)*num_sim);
  T_sim=(double *)gcemalloc(sizeof(double)*num_sim);
