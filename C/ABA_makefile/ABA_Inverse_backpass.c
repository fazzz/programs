...
void ABA_Inverse_backpass(ABI* abi,CLT *clt,double *Q,int numclut) {
  ...
  for (i=numclut-1;i>=0;--i) {
	...
	for (j=0;j<6;++j) {
      ...
      Q[i]=ABAb_cQ(clt[i].F,clt[i].IM,clt[i].Spacc,clt[i].Cofrc,clt[i].Spfrc,TMI,Fpart,clt[i].num_branch);
    }
    else {
...
double ABAb_cQ(double F[6], double IM[6][6], double Spacc[6],double Cofrc[6],double Spfrc[6], double TM[4][6][6],double Fparent[4][6],int num_branch){
  int i,j,nb;
  double Q;
