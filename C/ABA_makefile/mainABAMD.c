...
int main(int argc, char *argv[]) {
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (TERMMDOE==ON) ABA_update_Term(crd,delta_term,numatom);
    ...
    if (TERMMDOE==ON)
      solverABA_TermOn(qacc,qvel,clt,Q,frc,crd,numclut,numatom,q,q_NVT,qvel_NVT,&qacc_NVT,s_NVT,acc_Term,acc_Term2,vel_Term,KE,KEobj,MODE);
    ...
    if (TERMMDOE==OFF)
      ABA_calcKineE(&KE,&KEv,&PEv,KEobj,clt,crd,qvel,q_NVT,qvel_NVT,s_NVT,numclut,numatom,MODE,numclut);
    if (TERMMDOE==ON)
      ABA_calcKineE_TermOn(&KE,&KEv,&PEv,KEobj,clt,crd,qvel,q_NVT,qvel_NVT,s_NVT,vel_Term,numclut,numatom,MODE,numclut+6);

    if (i%interval==0) {
      fprintf(outputfile,"%lf %lf %lf %lf %lf \n",pot,KE,KEv,PEv,pot+KE+PEv+KEv);
      ABA_out_formated(outputfile2,pot,KE,i,dt);
