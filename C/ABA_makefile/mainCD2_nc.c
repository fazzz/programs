...
int main(int argc, char *argv[]) {
  ...
  int flag='P',flagcn='n',flagof='f',amberflag=OFF,flagna=OFF,Drestoutflag=OFF,crdflag=OFF,amberncflag=OFF;
  ...
  while((c=getopt(argc,argv,"hAIBCLPOKNcnmflD:t:i:p:o:d:k:a:"))!=-1) {
    switch(c) {
    case 'A':
      amberflag=ON;
      break;
  ...
  if (ambcrdflag==OFF && pdbflag==OFF) {
	...
	mync_open_inq_get_sh_MCD(inputfilename,numatom,0,1,1,&nc_id_MCD,crd_nc);
      }
    }
  }
    ...
    if (initialstep>0) {
      for (i=0;i<initialstep;++i) {
	if (flagof=='c') fprintf(outputfile,"%d ",i);
	mync_open_inq_get_sh_AMBER(inputfilename,numatom,0,1,1,&nc_id_MCD,crd_nc);
      }
    }
  }
    ...
    inputfile=efopen(inputfilename,"r");
    crd=(double *)gcemalloc(sizeof(double )*numatom*3);
    readPDB(inputfilename,PD,numatom);
    fclose(inputfile);
    for (i=0;i<numatom;++i)
      for (j=0;j<3;++j)
