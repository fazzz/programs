...
! just one of these is zero.
...
#ifdef CUDA
  !The CUDA code does not currently support modifications to the off diagonal
  !elements in the LJ terms. The frcmod and parmXX dat files do not support
  !this, that is LJ terms are specified in terms of atom types and the mixing
  !rules are then applied. Leap also does not support pairwise LJ terms but the
  ...
  !to save shared memory, currently recalculates the LJ parameters for the
  !individual atom types from the diagonal and then uses them. Thus if someone
  !modifies the off-diagonal elements in the prmtop then the GPU code will not
  !give the answer expected. We trap for this here by computing the LJ params
  !from the diagonal and checking all the off diagonal elements.
  !
  !The GPU code also only supports the default AMBER combining rules. That is
  ...
  do i = 1, ntypes
    do j = 1, i
      ...
      aij_tmp = 4.0d0 * (sigma_tmp(i) + sigma_tmp(j))**12 * epsilon_tmp(i) * epsilon_tmp(j)
      bij_tmp = 4.0d0 * (sigma_tmp(i) + sigma_tmp(j))**6 *  epsilon_tmp(i) * epsilon_tmp(j)
      !compare to a small difference here as absolute comparissons are unreliable
      !for floating point.
      if (abs(aij_tmp - gbl_cn1(loop_count)) > (1.0d-7*abs(aij_tmp)) ) then
        write(mdout, '(a)') 'CUDA (GPU): Prmtop appears to contain modified off-diagonal elements for VDW A coef.'
        write(mdout, '(a)') '            GPU code does NOT support such force field modifications.'
        write(mdout, '(a,f22.10,a,f22.10)') '            Found Aij = ', aij_tmp, ' expected ', gbl_cn1(loop_count)
        write(mdout, '(a,i4,a,i4,a,i6)') '            i = ', i, ' j = ', j, ' Matrix element = ', loop_count
        call mexit(mdout,1)
      end if
      if (abs(bij_tmp - gbl_cn2(loop_count)) > (1.0d-7*abs(bij_tmp)) ) then
        write(mdout, '(a)') 'CUDA (GPU): Prmtop appears to contain modified off-diagonal elements for VDW B coef.'
        write(mdout, '(a)') '            GPU code does NOT support such force field modifications.'
        write(mdout, '(a,f22.10,a,f18.10)') '            Found Bij = ', bij_tmp, ' expected ', gbl_cn2(loop_count)
        write(mdout, '(a,i4,a,i4,a,i6)') '            i = ', i, ' j = ', j, ' Matrix element = ', loop_count
        call mexit(mdout,1)
      end if
    end do
