...
__launch_bounds__(SM_3X_GBNONBONDENERGY1_THREADS_PER_BLOCK, SM_3X_GBNONBONDENERGY1_BLOCKS_MULTIPLIER)
#elif (__CUDA_ARCH__ >= 200)
__launch_bounds__(SM_2X_GBNONBONDENERGY1_THREADS_PER_BLOCK, SM_2X_GBNONBONDENERGY1_BLOCKS_MULTIPLIER)
#else
__launch_bounds__(SM_13_GBNONBONDENERGY1_THREADS_PER_BLOCK, SM_13_GBNONBONDENERGY1_BLOCKS_MULTIPLIER)
#endif
...
extern "C" void kCalculateGBNonbondForces1(gpuContext gpu)
{
    ...
    kCalculateGBNonbondForces1_kernel<<<gpu->GBNonbondEnergy1Blocks, gpu->GBNonbondEnergy1ThreadsPerBlock>>>();
...
{
    cudaFuncSetSharedMemConfig(kCalculateGBNonbondEnergy1_kernel, cudaSharedMemBankSizeEightByte);
    cudaFuncSetSharedMemConfig(kCalculateGBNonbondForces1_kernel, cudaSharedMemBankSizeEightByte);
}
...
extern "C" void kCalculateGBNonbondEnergy1(gpuContext gpu)
{
    ...
    kCalculateGBNonbondEnergy1_kernel<<<gpu->GBNonbondEnergy1Blocks, gpu->GBNonbondEnergy1ThreadsPerBlock>>>(); 
...
kReduceGBTemp7Energy_kernel()
{
    ...
    egb                                                 = sE[threadIdx.x];
    unsigned long long int val                          = llitoulli(lliroundd(egb * ENERGYSCALE));
    if (threadIdx.x == 0)
