...
{
#ifdef use_SPFP
    cudaMemset(gpu->sim.plliXYZ_q, 0, gpu->sim.XYZStride * sizeof(long long int));
#else
    if (gpu->bOddNLCells)
...
kPMEReduceChargeGridBuffer_kernel()
{
    ...
    while (pos < limit)
    {
        long long int value                     = cSim.plliXYZ_q[pos];
        PMEFloat value1                         = (PMEFloat)value * ONEOVERFORCESCALEF;
...
#ifdef use_SPFP
    kPMEReduceChargeGridBuffer_kernel<<<gpu->blocks, gpu->reduceForcesThreadsPerBlock>>>();
#else
    if (gpu->bOddNLCells)
...
#endif
#ifdef use_SPFP
kPMEFillChargeGridBuffer_kernel(int bOffset)
#else
...
#ifdef use_SPFP
    const int block                             = blockIdx.x + bOffset;
    uint2 cellStartEnd                          = {block * LOADSIZE, min((block + 1) * LOADSIZE, cSim.atoms)};
#else        
    // Load cell data
    if (threadIdx.x == 0)
    {
...
#ifdef use_SPFP
            unsigned long long int value        = llitoulli(lliroundf(FORCESCALEF * sAtom[pos1].tx[iOffsetX] * sAtom[pos1].ty[iOffsetY] * sAtom[pos1].tz[iOffsetZ]));
            atomicAdd((unsigned long long int*)&cSim.plliXYZ_q[gpos], value);
#else
            PMEFloat qvalue                     = pXYZ_q[gpos];        
            PMEFloat value                      = sAtom[pos1].tx[iOffsetX] * sAtom[pos1].ty[iOffsetY] * sAtom[pos1].tz[iOffsetZ];
