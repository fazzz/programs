...
kNLRemapLocalInteractions_kernel()
{
    ...
    while (pos < cSim.NMRDistanceOffset)
    {
        if (pos < cSim.NMRDistances)
        {
            ...
            atom.z                             += atom.x;
            atom.w                             += atom.y;         
            cSim.pImageNMRDistanceID[pos]       = atom;
        }
        pos                                    += blockDim.x * gridDim.x;   
    }
    while (pos < cSim.NMRAngleOffset)
    {
        ...
        if (pos < cSim.NMRAngles)
        {
            ...
            atom2.x                            += atom1.y;
            atom2.y                            += atom1.z;
            cSim.pImageNMRAngleID1[pos]         = atom1;
            cSim.pImageNMRAngleID2[pos]         = atom2;    
        }
        pos                                    += cSim.NMRDistanceOffset + blockDim.x * gridDim.x;
    }
    while (pos < cSim.NMRTorsionOffset)
    {
        ...
        if (pos < cSim.NMRTorsions)
        {
            ...
            atom2.z                            += atom1.z;
            atom2.w                            += atom1.w;
            cSim.pImageNMRTorsionID1[pos]       = atom1;
            cSim.pImageNMRTorsionID2[pos]       = atom2;
        }
        pos                                    += cSim.NMRAngleOffset + blockDim.x * gridDim.x;
    }  
}
...
__global__ void
#if (__CUDA_ARCH__ >= 300)
__launch_bounds__(SM_3X_NLBUILD_NEIGHBORLIST32_THREADS_PER_BLOCK, SM_3X_NLBUILD_NEIGHBORLIST_BLOCKS_MULTIPLIER)
#elif (__CUDA_ARCH__ >= 200)
__launch_bounds__(SM_2X_NLBUILD_NEIGHBORLIST32_THREADS_PER_BLOCK, SM_2X_NLBUILD_NEIGHBORLIST_BLOCKS_MULTIPLIER)
#else
__launch_bounds__(SM_13_NLBUILD_NEIGHBORLIST32_THREADS_PER_BLOCK, SM_13_NLBUILD_NEIGHBORLIST_BLOCKS_MULTIPLIER)
#endif
...
__global__ void
#if (__CUDA_ARCH__ >= 300)
__launch_bounds__(SM_3X_NLBUILD_NEIGHBORLIST16_THREADS_PER_BLOCK, SM_3X_NLBUILD_NEIGHBORLIST_BLOCKS_MULTIPLIER)
#elif (__CUDA_ARCH__ >= 200)
__launch_bounds__(SM_2X_NLBUILD_NEIGHBORLIST16_THREADS_PER_BLOCK + 64, SM_2X_NLBUILD_NEIGHBORLIST_BLOCKS_MULTIPLIER)
#else
__launch_bounds__(SM_13_NLBUILD_NEIGHBORLIST16_THREADS_PER_BLOCK, SM_13_NLBUILD_NEIGHBORLIST_BLOCKS_MULTIPLIER)
#endif
...
extern "C" void kNeighborListInitKernels(gpuContext gpu)
{
    ...
    {
        cudaFuncSetCacheConfig(kNLBuildNeighborListOrthogonalNTP32_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborListNTP32_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborListOrthogonal32_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborList32_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborListOrthogonalNTP16_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborListNTP16_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborListOrthogonal16_kernel, cudaFuncCachePreferL1);
        cudaFuncSetCacheConfig(kNLBuildNeighborList16_kernel, cudaFuncCachePreferL1);
    }
}
...
{ 
    if (gpu->sim.NLAtomsPerWarp == 32)
    {
        ...
        {
            if (gpu->sim.is_orthog)    
                kNLBuildNeighborListOrthogonalNTP32_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList32ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList32ThreadsPerBlock / GRID>>>();  
            else
                kNLBuildNeighborListNTP32_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList32ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList32ThreadsPerBlock / GRID>>>();         
        }
        else
        {
            if (gpu->sim.is_orthog)    
                kNLBuildNeighborListOrthogonal32_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList32ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList32ThreadsPerBlock / GRID>>>();  
            else
                kNLBuildNeighborList32_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList32ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList32ThreadsPerBlock / GRID>>>(); 
        }     
    }
    else
    {
        ...
        {
            if (gpu->sim.is_orthog)    
                kNLBuildNeighborListOrthogonalNTP16_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList16ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList16ThreadsPerBlock / GRID>>>();  
            else
                kNLBuildNeighborListNTP16_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList16ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList16ThreadsPerBlock / GRID>>>();         
        }
        else
        {
            if (gpu->sim.is_orthog)    
                kNLBuildNeighborListOrthogonal16_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList16ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList16ThreadsPerBlock / GRID>>>();  
            else
                kNLBuildNeighborList16_kernel<<<gpu->BNLBlocks, gpu->NLBuildNeighborList16ThreadsPerBlock, sizeof(uint) * gpu->sim.NLMaxExclusionsPerWarp * gpu->NLBuildNeighborList16ThreadsPerBlock / GRID>>>(); 
        }     
    }
