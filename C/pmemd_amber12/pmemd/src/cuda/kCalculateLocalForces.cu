...
extern "C" void kCalculateLocalForcesInitKernels(gpuContext gpu)
{
    ...
    cudaFuncSetCacheConfig(kCalculatePMELocalForcesVirial_kernel, cudaFuncCachePreferEqual);
}
...
#if (__CUDA_ARCH__ >= 300)
__launch_bounds__(SM_3X_CHARMMFORCES_THREADS_PER_BLOCK, 1)
#elif (__CUDA_ARCH__ >= 200)
...
#if (__CUDA_ARCH__ >= 300)
__launch_bounds__(SM_3X_NMRFORCES_THREADS_PER_BLOCK, 1)
#elif (__CUDA_ARCH__ >= 200)
__launch_bounds__(SM_2X_NMRFORCES_THREADS_PER_BLOCK, 1)
#else
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculateNMRForces_kernel(int step)
#include "kCNF.h"
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculateNMREnergy_kernel(int step)
#include "kCNF.h"
#undef NMR_ENERGY
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMRForces_kernel(int step)
#include "kCNF.h"
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMREnergy_kernel(int step)
#include "kCNF.h"
#undef NMR_ENERGY
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMRForcesVirial_kernel(int step)
#include "kCNF.h"
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMREnergyVirial_kernel(int step)
#include "kCNF.h"
#undef NMR_ENERGY
#undef NMR_VIRIAL
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculateNMRForcesFermi_kernel(int step)
#include "kCNF.h"
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculateNMREnergyFermi_kernel(int step)
#include "kCNF.h"
#undef NMR_ENERGY
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMRForcesFermi_kernel(int step)
#include "kCNF.h"
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMREnergyFermi_kernel(int step)
#include "kCNF.h"
#undef NMR_ENERGY
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMRForcesVirialFermi_kernel(int step)
#include "kCNF.h"
...
__launch_bounds__(SM_13_NMRFORCES_THREADS_PER_BLOCK, 1)
#endif
__global__ void kCalculatePMENMREnergyVirialFermi_kernel(int step)
#include "kCNF.h"
#undef NMR_ENERGY
#undef NMR_NEIGHBORLIST
#undef NODPTEXTURE
...
{
    int totalBlocks                 = (gpu->sim.NMRTorsionOffset + 63) / 64;
    ...
    if (gpu->bNMRInteractions)
    {
        if (!gpu->bECCSupport && (gpu->sm_version == SM_2X))
        {
            while (totalBlocks > 0)
            {
                ...
                {
                    if (gpu->sim.ntp > 0)
                        kCalculatePMENMRForcesVirialFermi_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                    else    
                        kCalculatePMENMRForcesFermi_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                }
                else
                    kCalculateNMRForcesFermi_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                LAUNCHERROR("kCalculateNMRForcesFermi");   
                totalBlocks        -= blocks;  
            }   
        }
        else
        {
            ...
            while (totalBlocks > 0)
            {
                ...
                {
                    if (gpu->sim.ntp > 0)
                        kCalculatePMENMRForcesVirial_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                    else
                        kCalculatePMENMRForces_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                }
                else
                    kCalculateNMRForces_kernel<<<blocks, 64>>>(gpu->NMRnstep);
    ...
    if (gpu->bNMRInteractions)
    {
        if (!gpu->bECCSupport && (gpu->sm_version == SM_2X))
        {
            while (totalBlocks > 0)
            {
                ...
                {
                    if (gpu->sim.ntp > 0)
                        kCalculatePMENMREnergyVirialFermi_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                    else
                        kCalculatePMENMREnergyFermi_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                }
                else
                    kCalculateNMREnergyFermi_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                LAUNCHERROR("kCalculateNMREnergyFermi");
        ...
        {    
            ...
            while (totalBlocks > 0)
            {
                ...
                {
                    if (gpu->sim.ntp > 0)
                        kCalculatePMENMREnergyVirial_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                    else
                        kCalculatePMENMREnergy_kernel<<<blocks, 64>>>(gpu->NMRnstep);
                }
                else
                    kCalculateNMREnergy_kernel<<<blocks, 64>>>(gpu->NMRnstep);
