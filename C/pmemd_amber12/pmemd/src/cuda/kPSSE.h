...
__shared__ PMEDouble sE[SM_3X_UPDATE_THREADS_PER_BLOCK];
#elif (__CUDA_ARCH__ >= 200)
__shared__ PMEDouble sE[SM_2X_UPDATE_THREADS_PER_BLOCK];
#else
__shared__ PMEDouble sE[SM_13_UPDATE_THREADS_PER_BLOCK];
#endif
#endif
...
#if (__CUDA_ARCH__ >= 300)
__shared__ ScalarSumVirial sV[SM_3X_UPDATE_THREADS_PER_BLOCK];
#elif (__CUDA_ARCH__ >= 200)
__shared__ ScalarSumVirial sV[SM_2X_UPDATE_THREADS_PER_BLOCK];
#else
__shared__ ScalarSumVirial sV[SM_13_UPDATE_THREADS_PER_BLOCK];
#endif
#endif

#if (__CUDA_ARCH__ >= 300) && defined(use_DPDP)
#define PREFACSIZE 2030
#elif (__CUDA_ARCH__ >= 300)
#define PREFACSIZE 4000
#elif (__CUDA_ARCH__ >= 200) && defined(use_DPDP)
...
__shared__ PMEFloat sRecipf[9];
    ...
    {    
        ...
        while (k3 < cSim.fft_z_dim)
        {
            ...
            vir_11                                 += lliroundf(eterm_struc2 * (vterm * mhat1 * mhat1 - (PMEFloat)1.0));
            vir_22                                 += lliroundf(eterm_struc2 * (vterm * mhat2 * mhat2 - (PMEFloat)1.0));
            vir_33                                 += lliroundf(eterm_struc2 * (vterm * mhat3 * mhat3 - (PMEFloat)1.0));
#else
            ...
            if ((k1 > 1) && (k1 <= cSim.nfft1))
            {
                ...
                vir_11                             += lliroundf(eterms_struc2s * (vterms * mhat1 * mhat1 - (PMEFloat)1.0));
                vir_22                             += lliroundf(eterms_struc2s * (vterms * mhat2 * mhat2 - (PMEFloat)1.0));
                vir_33                             += lliroundf(eterms_struc2s * (vterms * mhat3 * mhat3 - (PMEFloat)1.0));
#else
...
#ifdef PME_ENERGY
    unsigned long long int eer                      = llitoulli(lliroundd((PMEDouble)0.5 * sE[threadIdx.x] * ENERGYSCALE));
#endif
#ifdef PME_VIRIAL
#ifdef use_SPFP
...
#else        
    unsigned long long int vir11                    = llitoulli(lliroundd((PMEDouble)0.5 * sV[threadIdx.x].vir_11 * FORCESCALE));
    unsigned long long int vir22                    = llitoulli(lliroundd((PMEDouble)0.5 * sV[threadIdx.x].vir_22 * FORCESCALE));
    unsigned long long int vir33                    = llitoulli(lliroundd((PMEDouble)0.5 * sV[threadIdx.x].vir_33 * FORCESCALE));
#endif                  
#endif
