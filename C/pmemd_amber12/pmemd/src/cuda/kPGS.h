...
__shared__ volatile PMEFloat sPMEForce[3 * GRADSUMLOADSIZE];
    ...
    if (threadIdx.x < lastAtom)
    {
...
#endif // PME_VIRIAL
#if defined(use_SPFP) && defined(PME_VIRIAL)
        bool cond                           = (pos + threadIdx.x) >= cSim.maxProcessedAtom;
        PMEFloat fx1                        = cond ? fx : (PMEFloat)0.0;
        PMEFloat fy1                        = cond ? fy : (PMEFloat)0.0;
        PMEFloat fz1                        = cond ? fz : (PMEFloat)0.0;
        sPMEForce[threadIdx.x * 3]          = fx1;
        sPMEForce[threadIdx.x * 3 + 1]      = fy1;
        sPMEForce[threadIdx.x * 3 + 2]      = fz1;
#else
        ...
        unsigned int pos1                   = pos + threadIdx.x;
#ifdef use_SPFP 
        atomicAdd((unsigned long long int*)&cSim.pNBForceXAccumulator[pos1], llitoulli(lliroundf(fx * FORCESCALEF)));
        atomicAdd((unsigned long long int*)&cSim.pNBForceYAccumulator[pos1], llitoulli(lliroundf(fy * FORCESCALEF)));
        atomicAdd((unsigned long long int*)&cSim.pNBForceZAccumulator[pos1], llitoulli(lliroundf(fz * FORCESCALEF)));
#else 
    ...
    { 
        ...
        atomicAdd((unsigned long long int*)&cSim.pNBForceZAccumulator[pos1], llitoulli(lliroundf(fz * FORCESCALEF)));
#else
        cSim.pForceXBuffer[pos1]            = fx;
        cSim.pForceYBuffer[pos1]            = fy;
        cSim.pForceZBuffer[pos1]            = fz;
#endif   
    }
#endif // MPI
