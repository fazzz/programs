...
#ifdef CUDA
! IAT(n) must be >= 0 when the rst is read  because COM simulations are not
...
! Must transfer the character name to an integer variable, if necessary:
        ...
        ierr = 0
        call getnat(iat(1), iatnm(1), name, gbl_res_atms, nres, iout, ierr)
        call getnat(iat(2), iatnm(2), name, gbl_res_atms, nres, iout, ierr)
        call getnat(iat(3), iatnm(3), name, gbl_res_atms, nres, iout, ierr)
        call getnat(iat(4), iatnm(4), name, gbl_res_atms, nres, iout, ierr)
        if (ierr .eq. 1) then
          call mexit(iout, 1)
        end if
      end if
...
! values.
      ...
      if (iat(3) .le. 0) then
        ...
        if ((iat(1) .ge. 0 .and. iat(2) .ge. 0)) then
...
! Echo the restraint to the user, if the user has requested this:
      ...
      if (iuse .ne. 0) then
        ...
        if (iat(3) .le. 0) then
          ...
          if (iat(1) .gt. 0 .and. iat(2) .gt. 0) then
            write(iuse, 9040) name(iat(1)), iat(1), name(iat(2)), iat(2), nstep1, nstep2
          else if (iat(1) .gt. 0 .and. iat(2) .lt. 0) then
            write(iuse, 9040) name(iat(1)), iat(1), 'COM ', iat(2), nstep1, nstep2
          else if (iat(1) .lt. 0 .and. iat(2) .gt. 0) then
            write(iuse, 9040) 'COM ', iat(1), name(iat(2)), iat(2), nstep1, nstep2
          else
            write(iuse, 9040) 'COM ', iat(1), 'COM ', iat(2), nstep1, nstep2
          end if

        else if (iat(4) .le. 0) then

          write(iuse, 9041) name(iat(1)), iat(1), name(iat(2)), iat(2), name(iat(3)), &
                            iat(3), nstep1, nstep2
        else
          write(iuse, 9042) name(iat(1)), iat(1), name(iat(2)), iat(2), name(iat(3)), &
                            iat(3), name(iat(4)), iat(4), nstep1, nstep2
        end if
...
! If a group-type distance restraint was defined, echo the sorted group:
      ...
      convrt = 1.0d0
      if ((iat(3) .gt. 0 .or. iat(4) .gt. 0) .and. .not. jcoupl)  convrt = 180./PI
      rprt = rint*convrt
      rcurr = rcurr*convrt
...
! "cutoff" points (r2,r3). Use this as the value of the torsion in the report.
      ...
      if (iat(4) .gt. 0 .and. .not.jcoupl) then
...
! Convert r1->r4 to radians, if it is an angle:
      ...
      if ((iat(3) .gt. 0 .or. iat(4) .gt. 0) .and. .not.jcoupl) then
        r1 = r1/convrt
        r2 = r2/convrt
        r3 = r3/convrt
        r4 = r4/convrt
...
1003 continue
  ...
  if (iat(3) .le. 0) then
    if (iat(1) .gt. 0 .and. iat(2) .gt. 0) then
      write(iout, 9040) name(iat(1)), iat(1), name(iat(2)), iat(2), nstep1, nstep2
    else if (iat(1) .gt. 0 .and. iat(2) .lt. 0) then
      write(iout, 9040) name(iat(1)), iat(1), 'COM ', iat(2), nstep1, nstep2
    else if (iat(1) .lt. 0 .and. iat(2) .gt. 0) then
      write(iout, 9040) 'COM ', iat(1), name(iat(2)), iat(2), nstep1, nstep2
    else
      write(iout, 9040) 'COM ', iat(1), 'COM ', iat(2), nstep1, nstep2
    end if
  else if (iat(4) .le. 0) then
     write(iout, 9041) name(iat(1)), iat(1), name(iat(2)), iat(2), name(iat(3)), iat(3), &
                       nstep1, nstep2
  else
     write(iout, 9042) name(iat(1)), iat(1), name(iat(2)), iat(2), name(iat(3)), iat(3), &
                       name(iat(4)), iat(4), nstep1, nstep2
  end if
...
! and its energy contribution:
      ...
      iat(1:4) = (nmrat(1:4, i) / 3) + 1
      if (nmrat(1, i) .lt. 0) iat(1) = 0
      if (nmrat(2, i) .lt. 0) iat(2) = 0
...
!              j       for j-coupling constraint
      ...
      if (nmrat(1, i) .lt. 0 .and. nmrat(3, i) .lt. 0)  &
        iat(1) = nmrcom(2, - nmrat(3, i))

      if (nmrat(2, i) .lt. 0 .and. nmrat(4, i) .lt. 0)  &
        iat(2) = nmrcom(2, - nmrat(4, i))
...
#ifdef CUDA
  if ((jar .eq. 0) .and. ((.not. master) .or. (idmpav .le. 0) .or. (mod(nstep, idmpav) .ne. 0))) then
    return    
  end if
