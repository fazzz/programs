...
! Local variables:
  ...
  if (use_pme /= 0) then
    ...
    if ((mod(irespa, ene_avg_sampling) .eq. 0) .or. (mod(irespa, ntpr) .eq. 0) &
         .or. (iamd.gt.0) .or. (nmropt .ne. 0)) then
    ...
    if ((mod(irespa, ene_avg_sampling) .eq. 0) .or. (mod(irespa, ntpr) .eq. 0) &
            .or. (iamd.gt.0) .or. (nmropt .ne. 0)) then
      call gpu_ips_ene(uc_volume, pot_ene, virial, ekcmt)
      call update_time(nonbond_time)
      if (need_virials) then
        vir%molecular(1,1) = virial(1)
        vir%molecular(2,2) = virial(2)
...
! NMR restraints and forces -- download the coordinates and forces, calculate
! NMR contributions, then upload the forces back
  if (nmropt .ne. 0) then
...
! NMR restraints and forces -- download coordinates and forces, then upload the
! forces after we are done
  if (nmropt .ne. 0) then
    call gpu_download_crd(crd)
    call gpu_download_frc(frc)
    call nmr_calc(crd, frc, enmr, 6)
