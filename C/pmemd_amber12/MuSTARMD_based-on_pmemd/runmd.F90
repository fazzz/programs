...
! irest = 0:  General startup if not continuing a previous run:
  ...
  if (irest .eq. 0) then
    ...
    if (using_pme_potential) then
      ...
      call gpu_upload_crd(crd)
#endif
      call gb_force(atm_cnt, crd, frc, gb_pot_ene, irespa, .false.)
#ifdef CUDA
      call gpu_download_frc(frc)
#endif
...
! Sum up ekmh. The all_reduce is expensive, but only occurs once per run.
  ...
  if (local_remd_method .gt. 0) then
  ...
  do mdloop = 1, local_numexchg
    ...
    if (local_remd_method .gt. 0) then 
      ...
      if (using_pme_potential) then
        ...
        call gb_force(atm_cnt, crd, frc, gb_pot_ene, irespa, .true.)

        remd_ptot = gb_pot_ene%total

      end if
      
      if (local_remd_method .eq. 1) then
        call temperature_exchange(atm_cnt, vel, remd_ptot, 1, &
                                  si(si_kin_ene) / fac(1), print_exch_data, mdloop)
      else if (local_remd_method .eq. 3) then
        call hamiltonian_exchange(atm_cnt, my_atm_lst, crd, frc, remd_ptot, &
                                  gbl_img_atm_map, gbl_atm_img_map, 1)
        new_list = .true.
      end if

#ifndef CUDA
      ! We need to update both last_vel and vel after a REMD swap
      ! or the kinetic energy will not be calculated correctly. 
      ! On the gpu, this already happens in gpu_update, which
      ! is called before the kinetic energies are calculated.
      ! On the CPU, last_vel isn't updated until after the kinetic
      ! energies are calculated, so eke comes out wrong. This only
      ! only has an effect when velocities are scaled during an
      ! REMD swap. In this case, the kinetic energy is calculated
      ! incorrectly for the first step after an REMD swap. The
      ! kinetic energy is calculated correctly in all other
      ! instances.

      last_vel(:,:) = vel(:,:)
#endif
    end if ! remd_method .ne. 0
  ...
  ! calculated less frequently, depending on needs.
      ...
      if (using_pme_potential) then
        ...
        if (remd_method .eq. 0 .or. nstep .gt. 0) &
        call pme_force(atm_cnt, crd, frc, gbl_img_atm_map, gbl_atm_img_map, &
                       my_atm_lst, new_list, need_pot_enes, need_virials, &
                       pme_pot_ene, virial, ekcmt, pme_err_est)
#else
...
!Flush amdlog file containing the list of weights and energies at each step
  ...
  ! Energy archive:
        ...
        if (lout) then
          call prntmd(total_nstep, total_nstlim, t, si, fac, 7, .false., mdloop)
          if (nmropt .ne. 0) call nmrptx(6)
        end if
