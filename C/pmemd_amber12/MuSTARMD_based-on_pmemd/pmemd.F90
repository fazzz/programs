...
#ifdef CUDA
# ifdef MPI
    if (using_pme_potential .and. numtasks .lt. 2 .and. master) then
        write(6, *) &
            'MPI version of PMEMD must be used with 2 or more GPUs with PME.'
        call mexit(6,1)
    endif
# endif /* MPI */

! Initialize system
  call gpu_setup_system(natom, tol, ntf, ntb, ntp, ntt, vrand)
