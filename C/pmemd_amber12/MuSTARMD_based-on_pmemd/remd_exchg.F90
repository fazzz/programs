...
subroutine hamiltonian_exchange(atm_cnt, my_atm_lst, crd, frc, my_pot_ene_tot, &
...
! Local variables
...
#ifdef CUDA
  ! Update if we are using the GPU
  ...
  if (using_gb_potential) then
    ...
    call gb_force(atm_cnt, crd_temp, frc, my_new_gb_ene, irespa, .true.)
    ...
    call pme_force(atm_cnt, crd_temp, frc, gbl_img_atm_map, gbl_atm_img_map, &
  ...
  ! and then requires you to update temp0 at the end with an mpi_sendrecv call.
    ...
    if (i_do_exchg) then
      ...
      if (success) success_array(master_rank+1) = 1
...
#ifdef CUDA
  ! If we didn't succeed our exchange attempt, we need to pass the crds back
  ! to the GPU
