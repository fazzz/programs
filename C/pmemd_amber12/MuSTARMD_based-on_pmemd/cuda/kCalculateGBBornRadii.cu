...
__launch_bounds__(SM_2X_GBBORNRADII_THREADS_PER_BLOCK, 1)
#else
__launch_bounds__(SM_13_GBBORNRADII_THREADS_PER_BLOCK, 1)
#endif
...
__launch_bounds__(SM_2X_GBBORNRADII_THREADS_PER_BLOCK, 1)
#else
__launch_bounds__(SM_13_GBBORNRADIIIGB78_THREADS_PER_BLOCK, 1)
#endif
...
void kCalculateGBBornRadii(gpuContext gpu)
{
    if ((gpu->sim.igb == 7) || (gpu->sim.igb == 8))
        kCalculateGBBornRadiiIGB78_kernel<<<gpu->blocks, gpu->GBBornRadiiIGB78ThreadsPerBlock>>>();
    else
        kCalculateGBBornRadii_kernel<<<gpu->blocks, gpu->GBBornRadiiThreadsPerBlock>>>();
