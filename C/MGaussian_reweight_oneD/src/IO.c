...
int io_scanclust(FILE *clustfile, struct clust *clt) {
  ...
  fscanf(clustfile,"%d",&(clt->numclt));

  for (i=0;i<clt->numclt;++i) {
    fscanf(clustfile,"%d",&d);
    clt->nainiclt[i]=d;
  }
  for (i=0;i<clt->numclt;++i) {
    fscanf(clustfile,"%d",&d);
...
int io_scanconfwj(FILE *inputfile,int numatom,double *data,int flag){
  ...
  for (i=0;i<numatom;++i) {
    if (flag=='c')
      w=fscanf(inputfile,"%d",&d);
      if (w==-1) return -1;
    for (j=0;j<3;++j) {
      w=fscanf(inputfile,"%lf",&f);
      if (w==-1) return -1;
      data[i*3+j]=f;
    }
  }
...
int io_scanmatrix_ifv(FILE *inputfile,int numcolum,int numrow,int numinirow, int numfinrow,double **data){
  ...
  for (i=0;i<numcolum;++i) 
    for (j=0;j<numrow;++j) 
      if (j>=numinirow && j<=numfinrow ) fscanf(inputfile,"%lf",&data[j-numfinrow][i]);
...
int io_scancalphatraj_aw(FILE *inputfile,int numstep,int numatom,int numres,double *traj) {
  ...
  for (i=0;i<numstep;++i) {
	...
	for (k=0;k<3;++k) {
	  fscanf(inputfile,"%lf",&d);
	  traj[/*l*numres*3+i*3+j*/i*numres*3+l*3+k] = d*sqrt(AP.AMASS[j]);
	}
	++l;
      }
      else  for (k=0;k<3;++k) fscanf(inputfile,"%lf",&d);
    }
  }
  return 0;
}
...
int io_outputts_spdim(FILE *outputfile,int numstep,int numdimension,int spdim,double *data){
  int i,j,d;
