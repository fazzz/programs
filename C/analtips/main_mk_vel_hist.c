...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt(argc,argv,"hcan:w:i:"))!=-1) {
    switch(c) {
    case 'c':
      cutflag=ON;
      break;
  ...
  if (cutflag==ON)  getline(&line,&len,inputfile);
  ...
  for (i=0;i<numstep;++i) {
	...
	if (Amberflag==ON) {
	  vel[j*3]=vel[j*3]*20.455;
	  vel[j*3+1]=vel[j*3+1]*20.455;
	  vel[j*3+2]=vel[j*3+2]*20.455;
	}
...
int USAGE(char *progname) {
  printf("USAGE:\n");
  printf("[-c cutflag ON ]:\n");
  printf("[-A Amberflag ON ]:\n");
  printf("[-n normflag]:\n");
  printf("[-w width]:\n");
  printf("[-i initilal step]:\n");
  printf("[-h help]:\n");
  printf("%s numspatom numstep inputfilename parmfilename outputfilename histfilename\n",progname);
}
