...
subroutine final_gb_setup(atm_cnt, num_ints, num_reals)
...
! Local variables:
  ...
  if (irespa .lt. 2 .or. mod(irespa, nrespai) .eq. 0) &
  ...
  end if  !  i = mytaskid + 1, max_i, numtasks
...
#else
! "Accelerated" GB code. Target architecture is Itanium2 (ia64)
!  Roberto Gomperts
!  SGI
!  September 2007
!*******************************************************************************
...
! Arrays for GB_ACCELeration
  integer,          allocatable, save, private  :: ktmp(:,:), kktmp(:,:)
  integer,          parameter     :: kcases=10, kkcases=6

  integer, save, private          :: kcase, case_radii(kcases)
...
! Local variables:
  ...
  ! A few other variables and arrays to avoid branching
  ...
  do i = 1, max_i
    ...
    ! Start first outer loop
    ! Separate 4 combinations of gb_kappa and alpb

  if (gb_kappa .eq. 0.d0) then
    if (alpb.eq.1) then    ! gb_kappa == 0.0d0 && alpb == 1
    !dir$ ivdep
    do k = 1, icount
...
! Initialize to avoid branching in k=1,icount loop
      thi2 = 1.0d0
      if (igb .eq. 2 .or. igb .eq. 5 .or. igb .eq. 7) then
      ...
      kk1 = 0
         do k=1,icount
            ...
            if (dij > rgbmax - sj ) then
               case_radii(1) = case_radii(1) + 1
               ktmp(case_radii(1),1) = k

            else if( dij > 4.d0*sj ) then
               case_radii(2) = case_radii(2) + 1
               ktmp(case_radii(2),2) = k
            ...
            if( dij > ri+sj ) then
               vectmp2(kk1) = r2 - sj2
               vectmp4(kk1) = dij - sj
               case_radii(3) = case_radii(3) + 1
               ktmp(case_radii(3),3) = k
               kktmp(case_radii(3),1) = kk1
            else if ( dij > abs(ri-sj) ) then
               vectmp2(kk1) = dij + sj
               vectmp4(kk1) = ri
               case_radii(4) = case_radii(4) + 1
               ktmp(case_radii(4),4) = k
               kktmp(case_radii(4),2) = kk1
            else if ( ri < sj ) then
               vectmp2(kk1) = r2 - sj2
               vectmp4(kk1) = sj - dij
               case_radii(5) = case_radii(5) + 1
               ktmp(case_radii(5),5) = k
               kktmp(case_radii(5),3) = kk1
            else
               vectmp2(kk1) = 1.0d0
               vectmp4(kk1) = 1.0d0
            end if
...
!ddd         if (dij > rgbmax + si) cycle

       if ( dij <= rgbmax+si ) then
         if (dij > rgbmax - si) then
            case_radii(6) = case_radii(6) + 1
            ktmp(case_radii(6),6) = k
         else if( dij > 4.d0*si ) then
            case_radii(7) = case_radii(7) + 1
            ktmp(case_radii(7),7) = k
         end if
         ...
         if( dij <= 4.d0*si) then
            ...
            if( dij > rj+si) then
               vectmp5(kk2) = dij - si
               case_radii(8) = case_radii(8) + 1
               ktmp(case_radii(8),8) = k
               kktmp(case_radii(8),4) = kk2
            else if ( dij > abs(rj-si) ) then
               vectmp5(kk2) = rj
               case_radii(9) = case_radii(9) + 1
               ktmp(case_radii(9),9) = k
               kktmp(case_radii(9),5) = kk2
            else if ( rj < si ) then
               vectmp5(kk2) = si - dij
               case_radii(10) = case_radii(10) + 1
               ktmp(case_radii(10),10) = k
               kktmp(case_radii(10),6) = kk2
            else
               vectmp5(kk2) = 1.0d0
            end if
         end if
...
! Now the real loops de-referencing case_radii
! case1
!         if (dij <= rgbmax + sj) then
!            if ((dij > rgbmax - sj)) then
      !dir$ ivdep
...
!            else if( dij > ri+sj ) then
      ...
      do kcase = 1, case_radii(3)
         ...
         dij1i = vectmp1(k)
               ...
               kk1 = kktmp(kcase,1)
               reff_i = reff_i - 0.5d0*( sj/(r2-sj2) + 0.5d0*dij1i*vectmp4(kk1) )
      end do                    !  k = 1, icount
...
!            else if ( dij > abs(ri-sj) ) then
      ...
      do kcase = 1, case_radii(4)
         ...
         dij1i = vectmp1(k)
               ...
               kk1 = kktmp(kcase,2)
               theta = 0.5d0*ri1i*dij1i*(r2 + ri*ri -sj2)
               reff_i = reff_i - 0.25d0*( ri1i*(2.0d0-theta) &
                  - vectmp2(kk1) + dij1i*vectmp4(kk1) )
      end do                    !  k = 1, icount
...
!            else if ( ri < sj ) then
      ...
      do kcase = 1, case_radii(5)
         ...
         dij1i = vectmp1(k)
               ...
               kk1 = kktmp(kcase,3)
               reff_i = reff_i - 0.5d0*( sj/(r2-sj2) + 2.0d0*ri1i &
                  + 0.5d0*dij1i*vectmp4(kk1) )
      end do                    !  k = 1, icount
...
!         else if ( dij > abs(rj-si) ) then
      ...
      do kcase = 1, case_radii(9)
         ...
         dij1i = vectmp1(k)
            ...
            kk2 = kktmp(kcase,5)
            theta = 0.5d0*rj1i*dij1i*(r2 + rj*rj -si2)
            reff(j) = reff(j) - 0.25d0*( rj1i*(2.0d0-theta) &
                  - vectmp3(kk2) + dij1i*vectmp5(kk2) )
...
!         else if ( rj < si ) then
      ...
      do kcase = 1, case_radii(10)
         ...
         dij1i = vectmp1(k)
            ...
            kk2 = kktmp(kcase,6)
            reff(j) = reff(j) - 0.5d0*( si/(r2-si2) + 2.0d0*rj1i &
                  + 0.5d0*dij1i*vectmp5(kk2) )
