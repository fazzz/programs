...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt_long(argc,argv,"*hbnfs:v:t:a:i:x:N:{:}:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case '*':
      MODE=NVE;
      break;
  ...
  if (numatom < 10) {
    e.p_e=(double *)gcemalloc(sizeof(double)*numatom);
    e.p_LJ=(double *)gcemalloc(sizeof(double)*numatom);
  }
