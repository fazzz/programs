...
int main(int argc, char *argv[]) {
  ...
  char *trjfilename,*veloutfilename,*outputfilename,*outputfilename2,*rstfilename="rstcrd",*rstvelfilename="rstvel";
  char *umbfilename;

  char *logfilename="MD_pep_NH_MP1996_AAFF_Amber.log";

  FILE *inputfile,*velfile,*parmfile,*veloutfile;
  ...
  veloutfile=efopen(veloutfilename,"w");
  for (i=0;i<numstep;++i) {
    if (MODE==NVT) {
      if (UMBdihedmode==OFF) {
	if (EXCmode==OFF)
	...
	for (k=0;k<3;++k) crd_nc[j][k]=crd[j*3+k];
      ...
      for (j=0;j<numatom;++j) {
	for (k=0;k<3;++k) {
	  fprintf(veloutfile,"%8.5lf ",vel[j*3+k]);
	}
	fprintf(veloutfile,"\n");
      }
...
int USAGE(char *progname) {
  printf("USAGE:\n");
  printf("[-h] help \n");
  printf("%s [-h] inputfilename refcrdfilename parmfilename outputfilename outputfilename2 trjfilename veloutfilename\n",progname);
}
