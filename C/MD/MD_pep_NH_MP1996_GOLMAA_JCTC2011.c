...
int main(int argc, char *argv[]) {
  ...
  for (i=0;i<numstep;++i) {
    if (MODE==NVT)
      KE=MD_Propagetor_NH_MP1998_GOLMAA_JCTC2011(crd,vel,mass,&zeta,&V_zeta,Q,NfKT,numatom,&KEv,&PEv,dt,dt2,nc,wdt4,wdt2,&e,&f,&e_GOLM);
    else
    ...
    if (i%interval==0) {
      ...
      p_t=e_GOLM.p_t+0.5*e.p_LJ_14_t+e.p_d_t+e.p_a_t+e.p_b_t;
