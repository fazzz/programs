...
int main(int argc, char *argv[]) {
  ...
  predict=(double ***)gcemalloc(sizeof(double **)*numatom);
  correct=(double ***)gcemalloc(sizeof(double **)*numatom);
  for (i=0;i<numatom;++i) {
    predict[i]=(double **)gcemalloc(sizeof(double *)*3);
    correct[i]=(double **)gcemalloc(sizeof(double *)*3);
    for (j=0;j<3;++j) {
      predict[i][j]=(double *)gcemalloc(sizeof(double)*6);
      correct[i][j]=(double *)gcemalloc(sizeof(double)*6);
    }
  }
  ...
  for (i=0;i<numatom;++i) {
    for (j=0;j<3;++j) {
      correct[i][j][0]=crd[i*3+j];
      correct[i][j][1]=dt*v[i*3+j];
    }
  }
  ...
  if (MODEV==ON) {
    ...
    for (i=0;i<numatom;++i) {
      for (j=0;j<3;++j) {
	for (k=0;k<6;++k) {
	  fscanf(inputvelofile,"%lf",&correct[i][j][k]);
	}
      }
    }
    if (MODE==NVT)  for (i=0;i<M;++i) for (j=0;j<6;++j)  fscanf(inputvelofile,"%lf",&correct_zeta[i][j]);
  ...
  for (i=0;i<numstep;++i) {
	...
	for (l=0;l<6;++l) 
	  for (m=0;m<6;++m) 
	    predict[j][k][l] += Telar_Matrix[l][m]*correct[j][k][m];

    for (j=0;j<numatom;++j) {
      for (k=0;k<3;++k) {
	crd[j*3+k]=predict[j][k][0];
	v[j*3+k]=predict[j][k][1]/dt;
      }
    }
    if (MODE==NVT) 
