...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt_long(argc,argv,"*hs:v:e:n:t:a:i:j:k:x:N:b:c:{:}:1:2:3:4:5:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case '*':
      MODE=NVE;
      break;
  ...
  for (i=0;i<numstep;++i) {
    if (MODE==NVT)
      KE=MD_Propagetor_NH_MP1998_GOLMAA_PROTEINS2008_b_2013_08_31(crd,vel,mass,&zeta,&V_zeta,Q,NfKT,numatom,&KEv,&PEv,dt,dt2,nc,wdt4,wdt2,&e_GOLM,fact_b,fact_a,fact_t,fact_NC,fact_NNC);
    else 
