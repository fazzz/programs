...
void ABAm_backpass_TermOn_NH_new(double *qacc,ABI* abi,CLT *clt,int numclut,double *qvel,double gzi,double *gzi_vel,double tau2,double *acc_Term,double *acc_Term2,double *vel_Term,double Temp,double TempB) {
  ...
  //  for (i=0;i<numclut;++i) qacc[i] = 0.0; // 2014-08-13
  //  for (i=0;i<6;++i) cos(acc_Term2[i]); // 2014-08-13
  ...
  for (i=1;i<numclut;++i) {
    ...
    //      cos(abi[i].KG[j]); // 2014-08-13
    //      cos(clt[i].preSpacc[j]); // 2014-08-13
    //    }                    // 2014-08-13
    ...
    //    for(j=0;j<6;++j) {   // 2014-08-13
    //      cos(clt[nparent].Spacc[j]); // 2014-08-13
      //      cos(clt[0].Spacc[j]*0.001); // 2014-08-13
      //      cos(clt[1].Spacc[j]); // 2014-08-13
      //      cos(clt[2].Spacc[j]); // 2014-08-13
    //      cos(clt[i].Spacc[j]); // 2014-08-13
    //      cos(abi[i].KG[j]); // 2014-08-13
    //      cos(clt[i].preSpacc[j]); // 2014-08-13
    //      for (k=0;k<6;++k)  // 2014-08-13
    //	cos(clt[i].TM[k][j]); // 2014-08-13
      //      cos(clt[i].preSpacc[j]); // 2014-08-13
    //    }                    // 2014-08-13
    
    qacc[i] = 0.0;
    //    cos(qacc[i]); // 2014-08-13
    for(j=0;j<6;++j)
...
void ABAm_corBF_NH_TERM_new(double corABI[6][6],double corBF[6],double *acc_Term,double *acc_Term2,double *vel_Term){
  ...
  //invm2(temp,inv,6); // 2014-08-13
  invm5(temp,inv,6); // 2014-08-13
  //    for (i=0;i<6;++i) cos(acc_Term2[i]); // 2014-08-13
  //    for (i=0;i<6;++i) cos(acc_Term[i]); // 2014-08-13
    for(i=0;i<6;++i) {
      acc_Term[i]=0.0;
      acc_Term2[i]=0.0;
    }
    ...
    //    for (i=0;i<6;++i) for(j=0;j<6;++j) cos(inv[i*6+j]); // 2014-08-13
    //    for (i=0;i<6;++i) cos(acc_Term[i]); // 2014-08-13
...
void ABAm_mainpass_TermOn_NH_new(ABI* abi,CLT *clt,double *Q,int numclut,double *acc_Term,double *acc_Term2,double *vel_Term) {
  ...
  for (i=numclut-1;i>=0;--i) {
	...
	for (j=0;j<6;++j) {
      ...
      //      for (j=0;j<6;++j) {      // 2014-08-13
      //	cos(abi[i].corBF[j]);  // 2014-08-13
      //	for (k=0;k<6;++k) {      // 2014-08-13
      //	  cos(abi[i].corABI[j][k]); // 2014-08-13
      //	}                      // 2014-08-13
      //	cos(clt[i].Coacc[j]);  // 2014-08-13
      //	cos(clt[i].Cofrc[j]);  // 2014-08-13
      //	cos(clt[i].Spfrc[j]);  // 2014-08-13
      //  }                        // 2014-08-13
  ...
  //  for (i=0;i<6;++i) cos(abi[0].corBF[i]); // 2014-08-13
  //  for (i=0;i<6;++i) cos(abi[1].corBF[i]); // 2014-08-13
