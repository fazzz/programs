...
int myncL_open_inq_get_MCD(char *outfilename,int numatom,
  ...
  for (i = numini; i < numfin; i+=interval)  {
    ...
    //    nc_get_vara_double(nc_id_MCD->ncid,COORDINATE,          // 2014-06-20
    //		       nc_id_MCD->start_trj,nc_id_MCD->count_trj, // 2014-06-20
    //		       &trj[j][0][0]);                            // 2014-06-20
    nc_get_vara_double(nc_id_MCD->ncid,nc_id_MCD->trj_varid,      // 2014-06-20
		       nc_id_MCD->start_trj,nc_id_MCD->count_trj, // 2014-06-20
		       &trj[j][0][0]);                            // 2014-06-20
    nc_id_MCD->start_ene[0] = i;
    for (j=0;j<8;++j)
      //      nc_get_vara_double(nc_id_MCD->ncid,NAME_ENERGY_TERMS_L[j],                // 2014-06-20
      //			 nc_id_MCD->start_ene,nc_id_MCD->count_ene,&ene[k][j]); // 2014-06-20
      nc_get_vara_double(nc_id_MCD->ncid,nc_id_MCD->ene_term_varid[j],                  // 2014-06-20
			 nc_id_MCD->start_ene,nc_id_MCD->count_ene,&ene[k][j]);         // 2014-06-20
    ++k;
  }
...
char *NAME_SBAA_ENERGY_TERMS_L[6]= {
  "energy_total",
  "energy_cnb_t",
  "energy_nnb_t",
  "energy_d_t",
...
int myncL_create_def_SBAAMCD(char *outfilename,int numatom,
  ...
  for (i=0;i<NENERGY_SBAA_TERMS;++i) {
    enc_def_var((nc_id_MCD->ncid),NAME_SBAA_ENERGY_TERMS_L[i],NC_DOUBLE,NDIMS_ENERGY,nc_id_MCD->dimids_ene,&(nc_id_MCD->ene_term_varid[i]));
    nc_put_att_text((nc_id_MCD->ncid),(nc_id_MCD->ene_term_varid[i]),UNITS,strlen(ENE_UNIT),ENE_UNIT);
  }
...
int myncL_open_inq_get_ene_SBAAMCD(char *infilename,
  ...
  enc_inq_varid(nc_id_MCD->ncid,NAME_ENERGY_TERMS_L[0],&(nc_id_MCD->ene_term_varid[0]));
...
int myncL_put_crd_AMBER(struct my_netcdf_out_id_AMBER nc_id_MD,
  ...
  if((c=nc_put_vara_double((nc_id_MD.ncid),(nc_id_MD.trj_varid),&nc_id_MD.start_trj[0],&nc_id_MD.count_trj[0],&crd_nc[0][0])))
    ERR(c);
...
double *myncL_get_trj_aw2(char *inputfilename,double *mass,int IOMODE, int numatom,int *numstep){
  int s,i,j,k,l;
  ...
  for (i=0;i<*numstep;++i) {
    if (IOMODE==AMBER) myncL_open_inq_get_sh_AMBER(inputfilename,numatom,i,1,i+1,&nc_id_MD,crd_nc);
    ...
    traj=(double *)gcerealloc(traj,sizeof(double)*(*numstep)*(numatom)*3);
    for (j=0;j<numatom;++j)  {
      for (k=0;k<3;++k) {
	traj[i*(numatom)*3+j*3+k] = crd[j*3+k]*sqrt(mass[j]);
      }
    }
  }
