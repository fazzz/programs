...
int main(int argc, char *argv[]) {
  ...
  crd_Term=(double *)gcemalloc(sizeof(double)*numatom*3); // 2014-06-30
  ...
  trans_A_to_CN_terminal=(double *)gcemalloc(sizeof(double)*3*3); // 2014-06-29
  l_Term=(double *)gcemalloc(sizeof(double)*3); // 2014-06-29
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (TERMMODE==ON) ABA_update_Term(crd,delta_Term,numatom,clt,numclut,trans_A_to_CN_terminal,l_Term); // 2014-06-29
	...
	ABA_update_Term2(crd,crd_Term,numatom,trans_A_to_CN_terminal,l_Term);   // 2014-06-30
	for (j=0;j<numatom;++j) for (k=0;k<3;++k) crd_nc[j][k]=crd_Term[j*3+k]; // 2014-06-30
      }                                                                         // 2014-06-30
      else {                                                                    // 2014-06-30
