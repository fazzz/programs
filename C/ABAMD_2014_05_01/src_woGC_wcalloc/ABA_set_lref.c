...
void ABAs_local_reference(CLT *clt,int nNumClut_all,int num_atom_all,double *crd) {
  ...
  for (nNumClut=0;nNumClut<nNumClut_all;++nNumClut) {
    ...
    if (nNumClut == 0)  {
      //      clt[nNumClut].xoord=(double *)gcemalloc(sizeof(double)*num_atom_all*3); // 2014-07-22
      //      clt[nNumClut].xoord=(double *)emalloc(sizeof(double)*num_atom_all*3); // 2014-07-22 // 2014-09-05
      clt[nNumClut].xoord=(double *)calloc(num_atom_all*3,sizeof(double)); // 2014-07-22 // 2014-09-05
      sub_trans_A_to_CN(clt[nNumClut].trans_A_to_CN,clt[nNumClut].xoord,
			0,clt[nNumClut].origin_atom_a,
			-1,0,
			//			clt[nNumClutOfParent].origin_atom_a, // 2014-8-13
			0, // 2014-08-13
	...
	num+=clt[nNumClutdummy].num_atom_clust;
      ...
      sub_trans_A_to_CN(clt[nNumClut].trans_A_to_CN,
			clt[nNumClut].xoord,
			nNumClut,
			clt[nNumClut].origin_atom_a,
			nNumClutOfParent,
			/*clt[nNumClutOfParent].origin_atom_a*/ // 2011-11
			clt[nNumClutOfParent].terminal_atom_a[0],
			clt[nNumClutOfParent].origin_atom_a,
...
void sub_trans_A_to_CN(double Mtrans_A_to_CN[3][3], double *crd_local,
  ...
  //  mat=(double *)gcemalloc(sizeof(double)*num_atom*3); // 2014-07-22
  //  mat=(double *)emalloc(sizeof(double)*num_atom*3); // 2014-07-22 // 2014-09-05
  mat=(double *)calloc(num_atom*3,sizeof(double)); // 2014-07-22 // 2014-09-05
