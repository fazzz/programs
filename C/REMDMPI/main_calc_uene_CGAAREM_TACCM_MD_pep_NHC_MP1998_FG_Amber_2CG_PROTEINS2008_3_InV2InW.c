...
int main(int argc, char *argv[]) {
  ...
  int dummy;
  double *crd_AA,*crd_CG1,*crd_CG2,*Z,W;
  ...
  CGAAREMDreadInputs_calc_uene_1FG2CG_InV2InW(inputfile,Cdata.numatom,numRE,KZAAs,KZCG1s,KZCG2s,
					      trjfileAAname,trjfileCG1name,trjfileCG2name,
					      nc_id_MD_AA,nc_id_MD_CG1,nc_id_MD_CG2,trjfileZ,inputwfile);
  fclose(inputfile);
  ...
  for (i=0;i<numRE;++i) {
	...
	for (m=0;m<numRE;++m) {
	  ...
	  if (objmodeflag==FG) {
	    EZ=(EAAdiff-EAA)+ECG1+CGdata[0].e.p_t+ECG2+CGdata[1].e.p_t/*+*/-W;
