...
int main(int argc, char *argv[]) {
  ...
  for (i=0;i<numRE;++i) {
	...
	for (m=0;m<numRE;++m) {
	  ...
	  if (objmodeflag==CG)
	    ffLc_calcffandforce_AA(crd_AA,Cdata.numatom,&(AAdata.e),&(AAdata.f),ap_FG);
	  if (objmodeflag==FG)
	    ffLc_calcffandforce_AA(crd_CG,Cdata.numatom,&(CGdata.e),&(CGdata.f),ap_CG);

	  CE_TACCM_CGAA(crd_AA,crd_CG,Z,Cdata.numatom,Zdata.numZ,KZAAobj,KZCGobj,Zdata.pairs,pi,
			&EAAdiff,&ECGdiff,&EZdiff);
	  if (objmodeflag==CG) {
	    EZ=(ECGdiff-ECG)-1.0*(EAA+AAdata.e.p_d_t+AAdata.e.p_a_t+AAdata.e.p_b_t);
	    //	    EZ=(ECGdiff-ECG)+EAA+AAdata.e.p_d_t+AAdata.e.p_a_t+AAdata.e.p_b_t;
	  }
	  if (objmodeflag==FG) {
	    //	    EZ=(EAAdiff-EAA)+ECG+CGdata.e.p_d_t+CGdata.e.p_a_t+CGdata.e.p_b_t;
	    EZ=(EAAdiff-EAA)-1.0*(ECG+CGdata.e.p_d_t+CGdata.e.p_a_t+CGdata.e.p_b_t);
	  }
