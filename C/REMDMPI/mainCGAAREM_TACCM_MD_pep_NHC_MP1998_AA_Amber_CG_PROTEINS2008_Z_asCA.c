...
#include "REMDCGAA_TACCM_MPI_2_Amber_PROTEINS2008_Z_asCA.h"
...
int main(int argc, char *argv[]) {
  ...
  Zdata.index=(int *)gcemalloc(sizeof(int)*(Zdata.numZ));
  for (i=0;i<Zdata.numZ;++i) {
    fscanf(TACCMfile,"%d",&(Zdata.index[i]));
  }
  fclose(TACCMfile);
  ...
  Zdata.Z=(double *)gcemalloc(sizeof(double)*Zdata.numZ*3);
  Zdata.velZ=(double *)gcemalloc(sizeof(double)*Zdata.numZ*3);
  ...
  if ( vMode==OFF ) {
    ...
    MD_Generate_inivelo(Zdata.velZ,Zdata.massZ,Zdata.numZ,k_B*T0Z*UNITT);
  ...
  for (i=0;i<Cdata.numatom;++i) {
    if (i+1==Zdata.index[k]) {
      for (j=0;j<3;++j) {
	Zdata.Z[k*3+j]=AAdata.crd[i*3+j];
      }
      ++k;
    }
  }
  ...
  KBTZ=k_B*T0Z;  Zdata.NfKT=(3.0*Zdata.numZ+1)*KBTZ*UNITT;  Zdata.Q=tau2*KBTZ*UNITT*(3.0*Zdata.numZ);
  ...
  myncL_create_def_MCD(trjfilenameZ,Zdata.numZ,&(Zdata.trjfileZ));
  myncL_create_def_MCD(trjfilenameThetaAA,Zdata.numZ,&(Zdata.trjfilThetaAA));
  myncL_create_def_MCD(trjfilenameThetaCG,Zdata.numZ,&(Zdata.trjfilThetaCG));
  ...
  if (equflag==ON) {
    runTACCM_CGAA_MD_NHC_MP1998_Amber_PROTEINS2008_Z_asCA(// AA ///////////////////////////////////////////////////
							  ...
							  Zdata.Q,Zdata.NfKT,Zdata.T,
							  Zdata.numZ,Zdata.KZAA,Zdata.KZCG,Zdata.index,
    ...
    CE_TACCM_CGAA_Z_asCA(AAdata.crd,CGdata.crd,Zdata.Z,
			 (Cdata.numatom),Zdata.numZ,
			 Zdata.KZAA,Zdata.KZCG,Zdata.index,pi,
			 &EAA,&ECG,&EZ);

  }

  acc_ratio=MPI_CGAATREM_TACCM_MD_pep_NHC_MP1998_Amber_PROTEINS2008_Z_asCA(my_rank,num_procs,tag,&status,
