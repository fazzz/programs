...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt_long(argc,argv,"hv:n:E:s:e:a:i:x:T:t:B:m:X:p:c:E:P:A:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case 's':
  ...
  CGAAREMDreadInputs_test(inputfile,Cdata.numatom/numEXT,numRE,my_rank,
  ...
  for (i=1;i<numEXT;++i) {
    for (j=0;j<Cdata.numatom/numEXT*3;++j) {
      ...
      CGdata.crd[i*Cdata.numatom/numEXT*3+j]=CGdata.crd[j];
    }
  }
  ...
  ffL_set_calcffandforce(&(CGdata.e),&(CGdata.f));
  ...
  acc_ratio=MPI_CGAATREM_TACCM_MD_pep_NHC_MP1998_test(my_rank,num_procs,tag,&status,
