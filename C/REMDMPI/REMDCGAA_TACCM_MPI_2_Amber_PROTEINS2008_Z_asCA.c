...
double  CE_TACCM_CGAA_Z_asCA(double *crdAA,double *crdCG,double *Z,
			     ...
			     double KZAA,double KZCG,int *index,double pi,
  ...
  for (i=0;i<numatom;++i) {
    if (i+1==index[k]) {
      for (l=0;l<3;++l) {
	thetaAA[k*3+l]=crdAA[i*3+l];
	thetaCG[k*3+l]=crdCG[i*3+l];
      }
      ++k;
    }
  }
  ...
  *ECG=0.0;
  for (i=0;i<numZ;++i) {
    for (j=0;j<3;++j) {
      *EAA+=KZAA*(Z[i*3+j]-thetaAA[i*3+j])*(Z[i*3+j]-thetaAA[i*3+j]);
      *ECG+=KZCG*(Z[i*3+j]-thetaCG[i*3+j])*(Z[i*3+j]-thetaCG[i*3+j]);
    }
  }
  *EZ=*EAA+*ECG;
}
