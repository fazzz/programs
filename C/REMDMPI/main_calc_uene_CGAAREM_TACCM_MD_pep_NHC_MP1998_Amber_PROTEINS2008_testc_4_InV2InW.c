...
int main(int argc, char *argv[]) {
  ...
  FILE **inputwfile,**trjfileZ,**outputfile;
  ...
  inputwfile=(FILE **)gcemalloc(sizeof(FILE *)*numRE);
  KZAAs=(double *)gcemalloc(sizeof(double)*numRE);
  KZCGs=(double *)gcemalloc(sizeof(double)*numRE);

  inputfile=efopen(inputfilename,"r");
  CGAAREMDreadInputs_calc_uene_InV2InW(inputfile,Cdata.numatom,numRE,KZAAs,KZCGs,
				       trjfileAAname,trjfileCGname,
				       nc_id_MD_AA,nc_id_MD_CG,trjfileZ,inputwfile);
  fclose(inputfile);
  ...
  for (i=0;i<numRE;++i) {
	...
	fscanf(inputwfile[i],"%d %lf",&dummy,&W);
	++l;
      }
    }
	...
	for (m=0;m<numRE;++m) {
	  ...
	  if (objmodeflag==CG) {
	    //	    EZ=(ECGdiff-ECG)+EAA+AAdata.e.p_d_t+AAdata.e.p_a_t+AAdata.e.p_b_t;
	    EZ=(ECGdiff-ECG)-1.0*(EAA+/*+AAdata.e.p_t*/AAdata.e.p_e_t+AAdata.e.p_LJ_t+AAdata.e.p_e_14_t+AAdata.e.p_LJ_14_t+AAdata.e.p_d_t+AAdata.e.p_a_t+AAdata.e.p_b_t)/*+*/-W;
	    //	    EZ=-1.0*((ECGdiff-ECG)+EAA+AAdata.e.p_e_t+AAdata.e.p_LJ_t+AAdata.e.p_e_14_t+AAdata.e.p_LJ_14_t+AAdata.e.p_d_t+AAdata.e.p_a_t+AAdata.e.p_b_t)-/*+*/W;
	  }
	  if (objmodeflag==FG) {
	    //	    EZ=(EAAdiff-EAA)+ECG+CGdata.e.p_e_t+CGdata.e.p_LJ_t+CGdata.e.p_e_14_t+CGdata.e.p_LJ_14_t+CGdata.e.p_d_t+CGdata.e.p_a_t+CGdata.e.p_b_t+/*-*/W;
	    EZ=(EAAdiff-EAA)-1.0*(ECG+CGdata.e.p_e_t+CGdata.e.p_LJ_t+CGdata.e.p_e_14_t+CGdata.e.p_LJ_14_t+CGdata.e.p_d_t+CGdata.e.p_a_t+CGdata.e.p_b_t)/*+*/-W;
...
int USAGE(char *progname) {
  printf("USAGE:\n");
  printf("[-h] help \n");
  printf("%s [-h] inputfilename inputwfilename parametertrjfilename parmfilename TACCMfilename outputfilename\n",progname);
}
