...
int main(int argc, char *argv[]) {
  ...
  double zetaZ=0.0,V_zetaZ=0.0,QZ;
  double PEv,KEv;
  ...
  while((c=getopt_long(argc,argv,"*hbnf14LsEs:v:t:a:i:x:N:{:}:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case '*':
      MODE=NVE;
      break;
  ...
  z=(double *)gcemalloc(sizeof(double)*nump);
  velz=(double *)gcemalloc(sizeof(double)*nump);
  frcz=(double *)gcemalloc(sizeof(double)*nump);
  ...
  p_t=TACCM_calc_eff_f_dihed_type_AAFF_Amber(crd,numatom,
					     z,numz,
					     pairp,fcp,
					     frc,frc_z,
					     &e,&f);
  ...
  for (i=0;i<numstep;++i) {
    if (MODE==NVT)
      KE=MD_Propagetor_TACCM_NH_MP1998_AAFF_AMBER(crd,vel,mass,z,velz,massz,&zeta,&V_zeta,Q,NfKT,numatom,nump,&KEv,&PEv,dt,dt2,nc,wdt4,wdt2,&e,&f,pairp,fcp,&ez);
    else
