...
int main(int argc, char *argv[]) {
  ...
  while((c=getopt_long(argc,argv,"*fh+ovM:e:@:t:l:i:j:k:{:}:x:c:d:2:K:B:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case 'f':
      forceflag=OFF;
      break;
  ...
  for (i=0;i<numstep;++i) {
    ...
    ///////////////// TACCM //////////////////////
    TACCM_integ_pret_Z(predict_Z,correct_Z,Z,velZ,numZ,dt);
    ...
    for (j=0;j<numatom;++j)
      for (k=0;k<3;++k)
	// db   	frc[j*3+k]=e_GOLM.f_MB[j][k];
	frc[j*3+k]=0.0;

    ///////////////// TACCM //////////////////////
    // db  TACCM_CTheta(crd,numatom,theta,numZ,pairsZ,pi);
    ...
    ///////////////// TACCM //////////////////////
    TACCM_integ_cort_Z(predict_Z,correct_Z,accZ,Z,velZ,numZ,dt);
