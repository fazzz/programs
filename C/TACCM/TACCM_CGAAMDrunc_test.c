...
#include "TACCM_CGAAMDrunc_test.h"
...
double runTACCM_CGAA_MDc_NHC_MP1998_test(// AA /////////////////////////////////////////////////////////
  ...
  ffLc_calcffandforce(crdCG,numatom,&e_CG,&f_CG,ap_CG);
  ...
  for (i=0;i<numstep;++i) {
    ...
    PEAA=TACCM_MDc_Propagetor_NH_MP1998_AA_test(crdAA,velAA,mass,zetaAA,V_zetaAA,
						...
						dt,dt2,nc,wdt4,wdt2,
						&e,&f,ap_AA,Z,numZ,thetaAA,KZAA,pairs,PEZAA,pi);

    PECG=TACCM_MDc_Propagetor_NH_MP1998_AA_test(crdCG,velCG,mass,zetaCG,V_zetaCG,
						QCG,NfKTCG,numatom,&KECG,&KEvCG,&PEvCG,
						dt,dt2,nc,wdt4,wdt2,
						&e_CG,&f_CG,ap_CG,Z,numZ,thetaCG,KZCG,pairs,PEZCG,pi);
    ...
    if (i%interval==0) {
      ...
      PEAA=e.p_e_t+e.p_LJ_t+e.p_e_14_t+e.p_LJ_14_t+e.p_d_t+e.p_a_t+e.p_b_t;
      EtAA=PEAA+KEAA+PEvAA+KEvAA;
      fprintf(outputfileAA,"%d %8.4e %8.4e %8.4e %8.4e %8.4e %8.4e "
	                    ,i+1,PEAA,KEAA,KEvAA,PEvAA,EtAA,TAA);

      PECG=e_CG.p_e_t+e_CG.p_LJ_t+e_CG.p_e_14_t+e_CG.p_LJ_14_t+e_CG.p_d_t+e_CG.p_a_t+e_CG.p_b_t;
...
double runMDc_NHC_MP1998_Amber_AAFF_test(double *crd,double *vel, double *mass, int numatom,
  ...
  *varPE=0.0; *varKE=0.0; *varT=0.0;
  ...
  for (i=0;i<numstep;++i) {
    KE=MDb_Propagetor_NH_MP1998_AAFF_Amber_test(crd,vel,mass,
...
double runTACCM_MDc_NHC_MP1998_Amber_AAFF_test(double *crd,double *vel, double *mass, int numatom,
					       double *zeta,double *V_zeta, double Q,
					       struct potential e, struct force f, struct AmberParmL ap,
					       double T, double NfKT, int numstep, int interval,int *l,
					       double dt,double dt2,double wdt2[3] ,double wdt4[3] ,int nc,
  ...
  *varPEZ=0.0; *varKEZ=0.0; *varTZ=0.0;
  ...
  for (i=0;i<numstep;++i) {
    ...
    TACCM_MDc_Propagetor_NH_MP1998_AA_test(crd,vel,mass,
					   zeta,V_zeta,Q,NfKT,numatom,
					   &KE,&KEv,&PEv,
					   dt,dt2,nc,wdt4,wdt2,&e,&f,ap,Z,numZ,theta,Kapa,pairs,&PEZ,pi);
    ...
    TACCMb_MD_Propagetor_NH_MP1998_Z(Z,velZ,massZ,theta,
...
double CE_TACCMc_CGAA_test(double *crdAA,double *crdCG,double *Z, int numatom,int numZ,
...
double MDc_Propagetor_NH_MP1998_AAFF_Amber_test(double *crd,double *vel,double *mass,double *zeta,double *V_zeta,double Q,double NfKT,int numatom,double *KEv,double *PEv,double dt,double dt2,int nc,double wdt4[3],double wdt2[3], struct potential *e, struct force *f, struct AmberParmL ap) {
  int i,j,k;
  double *frc,KE;
...
double TACCM_MDc_Propagetor_NH_MP1998_AA_test(double *crd,double *vel,double *mass,
					      double *zeta,double *V_zeta,double Q,
  ...
  *PEZ=TACCM_calc_eff_FF_MD(crd,numatom,theta,Z,numZ,Kapa,frcZ,pairs,pi);
  ...
  for (i=0;i<numatom;++i)
    for (j=0;j<3;++j) 
      frc[i*3+j]=-(*f).f_b[i*3+j]+(*f).f_a[i*3+j]+(*f).f_d[i*3+j]+frcZ[i][j]+(*f).f_e[i*3+j]+(*f).f_LJ[i*3+j]+(*f).f_e_14[i*3+j]+(*f).f_LJ_14[i*3+j];
