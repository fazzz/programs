...
double runTACCM_CGAA_MDb_NHC_MP1998_test(// AA /////////////////////////////////////////////////////////
  ...
  ffLc_calcffandforce_AA(crdAA,numatom,&e,&f,ap_AA);
  ffLc_calcffandforce_AA(crdCG,numatom,&e_CG,&f_CG,ap_CG);
  ...
  for (i=0;i<numstep;++i) {
    ...
    PEAA=TACCM_MDb_Propagetor_NH_MP1998_AA_test(crdAA,velAA,mass,zetaAA,V_zetaAA,
						QAA,NfKTAA,numatom,&KEAA,&KEvAA,&PEvAA,
						dt,dt2,nc,wdt4,wdt2,
						&e,&f,ap_AA,Z,numZ,thetaAA,KZAA,pairs,PEZAA,pi);

    PECG=TACCM_MDb_Propagetor_NH_MP1998_AA_test(crdCG,velCG,mass,zetaCG,V_zetaCG,
...
double runTACCM_MDb_NHC_MP1998_Amber_AAFF_test(double *crd,double *vel, double *mass, int numatom,
  ...
  *varPEZ=0.0; *varKEZ=0.0; *varTZ=0.0;
  ...
  for (i=0;i<numstep;++i) {
    ...
    TACCM_MDb_Propagetor_NH_MP1998_AA_test(crd,vel,mass,
...
double MDb_Propagetor_NH_MP1998_AAFF_Amber_test(double *crd,double *vel,double *mass,double *zeta,double *V_zeta,double Q,double NfKT,int numatom,double *KEv,double *PEv,double dt,double dt2,int nc,double wdt4[3],double wdt2[3], struct potential *e, struct force *f, struct AmberParmL ap) {
  ...
  ffLc_calcffandforce_AA(crd,numatom,e,f,ap);
  for (i=0;i<numatom;++i)
    for (j=0;j<3;++j) 
      frc[i*3+j]=-(*f).f_b[i*3+j]+(*f).f_a[i*3+j]+(*f).f_d[i*3+j];
  for (i=0;i<numatom;++i) for (j=0;j<3;++j) vel[i*3+j]+=dt2*frc[i*3+j]/mass[i];
