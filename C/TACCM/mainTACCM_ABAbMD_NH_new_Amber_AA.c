...
int main(int argc, char *argv[]) {
  ...
  char *inputfilename,*inputvelofilename,*parmfilename,*clustfilename;
  char *trjfilename,*outputfilename,*outputfilename2,*rstfilename="rstcrd",*rstvelfilename="rstvel";

  char *logfilename="MD_pep_NH_MP1996_Amber_AA.log";

  FILE *inputfile,*velfile,*parmfile,*clustfile;
  ...
  while((c=getopt_long(argc,argv,"*fIh+ovM:m:@:t:l:i:j:k:{:}:x:K:B:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case 'f':
      forceflag=OFF;
      break;
  ...
  for (i=0;i<numstep;++i) {
    ...
    for (j=0;j<numatom;++j) {
      for (k=0;k<3;++k) {
	frc[j*3+k]+=f.f_e[j*3+k]+f.f_LJ[j*3+k]+f.f_e_14[j*3+k]+f.f_LJ_14[j*3+k];
      }
    }
    pot+=0.5*e.p_e_t+0.5*e.p_LJ_t+e.p_e_14_t+0.5*e.p_LJ_14_t;
