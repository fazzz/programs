...
double FVDIHED_force_dihed(double *crd,int numatom,double **f_d,int **pairs,double *dvdpsi, int numdihe){
  int i,j,k,l;
  int ii,jj,kk,ll;
  ...
  for (i=0;i<numdihe;++i) {
    ...
    for (j=0;j<3;++j) {
      fi[j] = -dvdpsi[i]*lkj*m[j]/(lm*lm);
      fl[j] =  dvdpsi[i]*lkj*n[j]/(ln*ln);
