...
int main(int argc, char *argv[]) {
  ...
  nnan=(int *)gcemalloc(sizeof(int)*1);
  ...
  while ( (c=getc(inpfile))!=-1 ) {
	...
	if (nl==0) {
	  ...
	  nnan=(int *)gcerealloc(nnan,sizeof(int)*n);
	  nnan[n]=0;
	}
	nnan[n]+=1;
	++n;
	...
	if ( vflag == ON ) {
      ...
      ave[n-1]=((nl-nnan[n-1])*ave[n-1]+v)/((nl-nnan[n-1])+1);
      if ( vflag == ON )
	var[n-1]=((nl-nnan[n-1])*var[n-1]+v*v)/((nl-nnan[n-1])+1);
    }
  }
