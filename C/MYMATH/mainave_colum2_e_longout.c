...
int main(int argc, char *argv[]) {
  ...
  if (oflag==ON) {
    for (i=0;i<ncmax;++i) {
      printf("%24.20e ",ave[i]);
      if ( vflag == ON )
      	printf("%24.20e ",sqrt(var[i]-ave[i]*ave[i]));
      //	printf("%24.20e ",sqrt(var[i]));
      printf("\n");
    }
  }
  else {
    ...
    for (i=0;i<ncmax;++i) fprintf(outfile1,"%24.20e ",ave[i]);
    if ( vflag == ON )
      for (i=0;i<ncmax;++i) fprintf(outfile2,"%24.20e ",sqrt(var[i]-ave[i]*ave[i]));
    fclose(inpfile);
    fclose(outfile1);
