...
char NAMERINGRES[NUMRINGRES*3]="HISTYRPHETRPPRO";
...
int make_bp_v2(int numatom,int *bp,int **bp_f , int *numb, char *name_atom_list,char *name_res_list, int aromaflag, int flagtoponon) {
  ...
  if (flagtoponon==ON) // 0811
    close_ring_v2(numatom,bp_f,numb,name_atom_list,name_res_list);
...
int close_ring_v2(int numatom,int **bp_f , int *numb, char *name_atom_list, char *name_res_list) {
  ...
  int n;
  int flagrng=OFF;
  char nameres[3],namerescomp[3];

  for (i=0;i<numatom;++i) totalnumb+=numb[i];

  for (i=0;i<numatom;++i) {
    flagrng=OFF;
    for (j=0;j<3;++j) nameres[j]=name_res_list[i*4+j];
    for (n=0;n<NUMRINGRES;++n) {
      for (j=0;j<3;++j) namerescomp[j]=NAMERINGRES[n*4+j];
      if(strncmp(nameres,namerescomp,3)==0) {
	flagrng=ON;
	break;
      }
    }
