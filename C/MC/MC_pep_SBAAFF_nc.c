...
int main(int argc, char *argv[]) {
  ...
  SBAAff_set_parameters_default(&ene_trial);

  SBAAff_set_calcff(&ene,crd,numatom);
  SBAAff_set_calcff(&ene_trial,crd,numatom);
  ...
  fprintf(accplog,"#        a        p/n",i+1,c,accp);
  l=0;
  for (i=0;i<numstep;++i) {
    for (j=0;j<numatom;++j) {
      for (k=0;k<3;++k)
	...
	for (k=0;k<3;++k) 
      ...
      accp=accp/(i+1);
      fprintf(accplog,"%8.3d %8.3d %8.3lf/n",i+1,c,accp);
