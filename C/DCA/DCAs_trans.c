...
void DCAs_trans_mainpass(int stref,int endref, CLT *clt,
  ...
  DCAs_mak_transMat(transMat,clt,stref,endref);

  DCAs_trans_b(bA1_t,transMat,bA1);
  DCAs_trans_b(bA2_t,transMat,bA2);

  DCAs_trans_P(PA1_t,transMat,PA1);
  DCAs_trans_P(PA2_t,transMat,PA2);
  DCAs_trans_P(PA21_t,transMat,PA21);
...
void DCAs_mak_transMat(double* transMat,CLT *clt,int nstart, int nend){
  ...
  for (i=0;i<6;++i)
    for (j=0;j<6;++j)
      transMat[i*6+j]=clt[nstart].TransMatrix[i][j];
      
  for (n=clt[nstart].nNumClutOfParent-1;n!=nend;n=clt[n].nNumClutOfParent-1) {
    for (i=0;i<6;++i)
      for (j=0;j<6;++j)
	temp[i*6+j]=clt[n].TransMatrix[i][j];
