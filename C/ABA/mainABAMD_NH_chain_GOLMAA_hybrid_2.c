...
int main(int argc, char *argv[]) {
  ...
  GOLMAA_hybrid_ff_set_calcff_2(&e_GOLM,refcrd,numatom,numres,e.parm.indexnb,e.parm.numnb);
  ...
  ABANH_chain_set_new(tau,&tau2,Q_NH,M,DOF,KBT,correct_zeta);
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (TERMMODE==ON) 
      ABA_calcKineE_TermOn_new(&KE,clt,crd,qvel,vel_Term,numclut,numatom);
    else 
      ABA_calcKineE_new(&KE,clt,crd,qvel,numclut,numatom);
    T=KE/(DOF*k_B)*2.0;

    if (TERMMODE==ON) 
      solverABA_TermOn_NH_chain(qacc,qvel,clt,Q,frc,crd,numclut,numatom,acc_Term,acc_Term2,vel_Term,zeta_vel,zeta_acc,M,DOF,KBT,Q_NH,tau2,T,Tobj);
    else 
      solverABA_NH_chain(qacc,qvel,clt,Q,frc,crd,numclut,numatom,zeta_vel,zeta_acc,M,DOF,KBT,Q_NH,tau2,T,Tobj);
    ...
    if (MODE==NVT) ABANH_chain_update_cort(zeta,zeta_vel,zeta_acc,predict_zeta,correct_zeta,M,dt);
