...
int main(int argc, char *argv[]) {
  ...
  char *inputfilename,*refcrdfilename,*inputvelofilename,*parmfilename,*clustfilename;
  ...
  FILE *inputfile,*refcrdfile,*velfile,*parmfile,*clustfile;
  ...
  while((c=getopt_long(argc,argv,"*h+ovM:e:@:t:l:i:j:k:{:}:x:c:",long_opt,&opt_idx))!=-1) {
    switch(c) {
    case '*':
      MODE=NVE;
      break;
  ...
  for (i=0;i<numclut;++i) qvel[i]=/*0.10*/0.00;
  ...
  if (MODEV==OFF) 
    ;
    //    KE=ABA_Generate_inivelo(clt,crd,qvel,vel_Term,numclut,numatom,TERMMODE,KBT*UNITT);
  else
    ABAs_restat_read_new(inputvelofilename,numclut,correct,correct_Term,correct_Term2,correct_gzi,MODE,TERMMODE);
  //    ABAs_restat_read_chain(inputvelofilename,numclut,correct,correct_Term,correct_Term2,correct_zeta,M,MODE,TERMMODE);
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (TERMMODE==ON) 
      ABA_integ_pret_Term(predict_Term,predict_Term2,correct_Term,correct_Term2,vel_Term,delta_Term,dt);
    if (MODE==NVT) 
      ABANH_update_pret_new(&gzi,predict_gzi,correct_gzi,&s,&s_vel,predict_s,correct_s,dt);
    //      ABANH_chain_update_pret(zeta,zeta_vel,predict_zeta,correct_zeta,M,dt);
    ...
    for (j=0;j<numatom;++j)
      for (k=0;k<3;++k)
	frc[j*3+k]=(e_GOLM).f_natatt[j][k]+(e_GOLM).f_repul[j][k]+(e_GOLM).f_d[j][k];
	//    	frc[j*3+k]=(e_GOLM).f_t[j][k];
    ...
    if (TERMMODE==ON) 
      ...
      solverABA_NH_new(qacc,qvel,clt,Q,frc,crd,numclut,numatom,q,gzi,&gzi_vel,s,&s_vel,tau2,T,Tobj);
    ...
    //    ABANH_chain_update_cort(zeta,zeta_vel,zeta_acc,predict_zeta,correct_zeta,M,dt);
    ABA_update(clt,crd,qrot,numclut,numatom);
    //    if (TERMMODE==ON) ABA_update_Term(crd,delta_Term,numatom);

    if (TERMMODE==ON)
      ABA_calcKineE_TermOn_new_simp(&KE,clt,crd,qvel,vel_Term,numclut,numatom);
    else
      ABA_calcKineE(&KE,clt,crd,qvel,numclut,numatom);
    if (MODE==NVT)
      ABANH_calcKE_new(gzi,s,s_vel,Q_NH,KEobj,&PEv,&KEv);
    //      ABANH_chain_calcKE_new(zeta,zeta_vel,Q_NH,M,DOF,KBT,&PEv,&KEv);

    if (i%interval==0) {
      E_t=e_GOLM.p_natatt_t+e_GOLM.p_repul_t+e_GOLM.p_d_t+KE+KEv+PEv;
      p_t=e_GOLM.p_natatt_t+e_GOLM.p_repul_t+e_GOLM.p_d_t;
  ...
  //  ABAs_restat_write_vel_chain(rstvelfilename,numclut,correct,correct_Term,correct_Term2,correct_zeta,M,MODE,TERMMODE);
