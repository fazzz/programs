...
int main(int argc, char *argv[]) {
  ...
  // 2014/02/03
  double rmsd_force,rmsd_Q;
  double ave_rmsd_force,ave_rmsd_Q;
  // 2014/02/03
  ...
  char *check_forceandTorquefilename;
  FILE *check_forceandTorquefile;

  // 2014/02/03
  char *check_forceandTorquefilename_rmsd;
  FILE *check_forceandTorquefile_rmsd;
  // 2014/02/03
  ...
  trjfilename       = *++argv;
  check_forceandTorquefilename = *++argv;
  // 2014/02/03
  check_forceandTorquefilename_rmsd = *++argv;
  ...
  check_forceandTorquefile=efopen(check_forceandTorquefilename,"w");
  // 2014/02/03
  check_forceandTorquefile_rmsd=efopen(check_forceandTorquefilename_rmsd,"w");
  // 2014/02/03
  ...
  for (i=0;i<numstep;++i) {
    ...
    if (i==0) {
      maxabs_force_trj=maxabs_force;
      index_atom_trj=index_atom;
      maxabs_Q_trj=maxabs_Q;
      index_clust_trj=index_clust;
    }
    else {
      if (maxabs_force_trj<maxabs_force) {
	maxabs_force_trj=maxabs_force;
	index_atom_trj=index_atom;
      }

      if (maxabs_Q_trj<maxabs_Q) {
	maxabs_Q_trj=maxabs_Q;
	index_clust_trj=index_clust;
      }
    }
    ...
    // 2014/02/03
    check_rmsd_forceandTorque(numatom,numclut,frc,Q,&rmsd_force,&rmsd_Q);
    ave_rmsd_force=(ave_rmsd_force*i+rmsd_force)/(i+1);
    ave_rmsd_Q=(ave_rmsd_Q*i+rmsd_Q)/(i+1);
    // 2014/02/03
  ...
  fprintf(check_forceandTorquefile,"%4d %24.20e %4d %24.20e \n",index_atom_trj+1,maxabs_force_trj,index_clust_trj+1,maxabs_Q_trj);
  // 2014/02/03
  fprintf(check_forceandTorquefile_rmsd,"%24.20e %24.20e \n",ave_rmsd_force,ave_rmsd_Q);
  // 2014/02/03

  fclose(check_forceandTorquefile);
  // 2014/02/03
  fclose(check_forceandTorquefile_rmsd);
  // 2014/02/03
  nc_close((nc_id.ncid));
