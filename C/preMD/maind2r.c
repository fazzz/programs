...
int main(int argc, char *argv[]) {
  ...
  int flag,joinflag,flagsof,flagnum;
  ...
  if ( typeflag==ATOM ) {
    fscanf(inputfile,"%d",&numd2r);
    d2r=(int *)gcemalloc(sizeof(int)*numd2r*2);
    for (i=0;i<numd2r;++i) {
      for (j=0;j<2;++j) {
	fscanf(inputfile,"%d",&d2r[i*2+j]);
      }
      if (d2r[i*2+0] >= d2r[i*2+1]) {
	printf("error\n");
	exit(1);
      }
    }
  }
  else if ( typeflag==RESIDUE ) {
    fscanf(inputfile,"%d",&numresdummy);
    residdummy=(int *)gcemalloc(sizeof(int)*numresdummy);
    for (i=0;i<numresdummy;++i) {
      fscanf(inputfile,"%d",&residdummy[i]);
    }
