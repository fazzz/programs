...
int *readd2rcinput(FILE *inputfile, int *numres, int specifymove) {
  int i,j,k,d;
  int c;
...
void writed2rcoutput(FILE *clustfileout,CLTb *clt, int numclut){
  int i,j,k,l;
...
double trans_d2rc(CLTb *clt, int *numclut, int **d2r, int numd2r) {
  ...
  for (i=0;i<numd2r;++i) {
	...
	/* for (j=0;j<(*numclut);++j) ABAb_setJoin(clt,j,joinflag);	   */
        /*******************************************************************/
        clt[0].join=0;
	for(j=1; j<(*numclut); ++j) ABAb_setJoin(clt,j);
