...
int main(int argc, char *argv[]) {
  ...
  for (i=0;i<dat.n_sim;++i) {
    for (j=0;j<dat.K;++j) {
      dat.C_ik[i][j]=Simpson_integ_oneD_GBRW_C_GB(num_Simpson,minv,maxv,dat.x_i[i],dat.x_k[j],bk_i[i],dat.h);
    }
  }
  ...
  for (i=0;i<dat.K;++i) {
    dat.A[i]=Simpson_integ_oneD_GBRW_GB(num_Simpson,minv,maxv,dat.x_k[i],dat.h);
    dat.A[i]=1.0/dat.A[i];
  }
  ...
  sum=0.0;
  for (i=0;i<dat.K;++i) {
    sum+=w_k[i]*dat.A[i]*Simpson_integ_oneD_GBRW_GB(num_Simpson,minv,maxv,dat.x_k[i],dat.h);
  }
