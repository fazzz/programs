...
#include "Simpson_integ_GBRW_2.h"
#include "EF.h"
...
int main(int argc, char *argv[]) {
  ...
  dat.A_i=(double *)gcemalloc(sizeof(double *)*dat.n_sim);

  dat.B_ik=(double **)gcemalloc(sizeof(double *)*dat.n_sim);
  for (i=0;i<dat.n_sim;++i) dat.B_ik[i]=(double *)gcemalloc(sizeof(double)*dat.K);
  ...
  for (i=0;i<dat.n_sim;++i) {
    dat.C_ik[i][j]=Simpson_integ_oneD_GBRW_A(num_Simpson,minv,maxv,dat.x_i[i],dat.x_k[j],bk_i[i],dat.h,dat.beta);
  }

  for (i=0;i<dat.n_sim;++i) {
    for (j=0;j<dat.K;++j) {
      dat.B_ik[i][j]=Simpson_integ_oneD_GBRW_B(num_Simpson,minv,maxv,dat.x_i[i],dat.x_k[j],bk_i[i],dat.h,dat.beta);
    }
  }
  ...
  for (i=0;i<N_bin;++i) {
    ...
    for (j=0;j<dat.K;++j) {
      ...
      p=w_k[j]*exp(-0.5*delta*delta/dat.h);
      pmf+=p;
    }
