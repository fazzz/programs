    ...
    const lbfgsfloatval_t step)
{
  ...
  C_ik=(double **)gcemalloc(sizeof(double *)*N);
  for (i=0;i<N;++i) C_ik[i]=(double *)gcemalloc(sizeof(double *)*K);
  for (i=0;i<N;++i) for (j=0;j<K;++j) C_ik[i][j]=(*dat).C_ik[i][j];
  ...
  A=(double *)gcemalloc(sizeof(double)*K);
  for (k=0;k<K;++k)
    A[k]=(*dat).A[k];
