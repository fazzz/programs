...
double cv(double r1[5][3],double r2[5][3],double e1[2], double e2[2], double *p1, double *h1,double *p2, double *h2, double k1, double k2, int sflag, int flag, struct potential *energy,double ini);
...
int main(int argc, char *argv[]) {
  int i,j,k,l;
  int c,sflag='O',flag='1',eflag='E';
  int numstep,interval;

  double f,beta,delta;
  double ini;
  double dx=0.01,dex=0.00001,k1=0.0,k2=0.0;
  ...
  while((c=getopt(argc,argv,"OT12EF"))!=-1) {
    switch(c) {
    case 'O':
      sflag='O';
      break;
  ...
  ini=sqrt((e1[0]-e2[0])*(e1[0]-e2[0])+(e1[1]-e2[1])*(e1[1]-e2[1]));

  v=cv(r1,r2,e1,e2,&p1,&h1,&p2,&h2,k1,k2,sflag,flag,&ene,ini);
  for (i=0;i<5;++i) {
    for (j=0;j<3;++j) {
      r1_trial[i][j]=r1[i][j];
      r2_trial[i][j]=r2[i][j];
  ...
  for (i=0;i<numstep;++i) {
	...
	r1_trial[j][k]=r1[j][k]+dx*Box_Muller(i,0.0,1.0);
      v_trial=cv(r1_trial,r2_trial,e1_trial,e2_trial,&p1_trial,&h1_trial,&p2_trial,&h2_trial,k1,k2,sflag,flag,&ene,ini);
      delta=v_trial-v;
    ...
    if (eflag=='O') {
      for (k=0;k<2;++k)
	e1_trial[k]=e1[k]+dex*Box_Muller(i,0.0,1.0);
      v_trial=cv(r1_trial,r2_trial,e1_trial,e2_trial,&p1_trial,&h1_trial,&p2_trial,&h2_trial,k1,k2,sflag,flag,&ene,ini);
      delta=v_trial-v;
      if((c=Metropolis(beta*delta))==1) {
	for (k=0;k<2;++k)
	...
	v=v_trial;
      ...
      if (eflag=='O') {
	for (k=0;k<2;++k)
	  e2_trial[k]=e2[k]+dex*Box_Muller(i,0.0,1.0);
	v_trial=cv(r1_trial,r2_trial,e1_trial,e2_trial,&p1_trial,&h1_trial,&p2_trial,&h2_trial,k1,k2,sflag,flag,&ene,ini);
	delta=v_trial-v;
	if((c=Metropolis(beta*delta))==1) {
	  for (k=0;k<2;++k)
	...
	if (e2[k]>pi)
	  e2[k]-=2.0*pi;
      fprintf(dihfile,"%d %12.8lf %12.8lf ",i+1,p1*180.0/pi,h1*180.0/pi);
      if (flag=='2')
	fprintf(dihfile," %12.8lf %12.8lf\n",p2*180.0/pi,h2*180.0/pi);
      else
	fprintf(dihfile,"\n");
      fprintf(eatafile,"%d %12.8lf %12.8lf ",i+1,e1[0]*180.0/pi,e1[1]*180.0/pi);
      if (flag=='2')
	fprintf(eatafile,"%12.8lf %12.8lf\n",e2[0]*180.0/pi,e2[1]*180.0/pi);
      else
	fprintf(eatafile,"\n");
      fprintf(enefile,"%d %12.8lf %12.8lf ",i+1,v,ene.v[0]);
      if (flag=='2')
	fprintf(enefile,"%12.8lf %12.8lf %d\n",ene.v[1],ene.w,c);
      else
	fprintf(enefile,"%d %12.8lf\n",c,ene.w);
      fprintf(ene_detailfile,"%d %12.8lf %12.8lf %12.8lf %12.8lf %12.8lf %12.8lf %12.8lf %12.8lf %12.8lf %12.8lf \n",i+1,v,ene.bond[0],ene.bond[1],ene.angl[0],ene.angl[1],ene.dihd[0],ene.angl[1],ene.eata[0],ene.eata[1],ene.eata[2]);
    }
  }
...
double cv(double r1[5][3],double r2[5][3],double e1[2], double e2[2], double *p1, double *h1,double *p2, double *h2, double k1, double k2, int sflag, int flag, struct potential *energy,double ini) {
  ...
  *h2=dih(r2[1],r2[2],r2[3],r2[4]);
  ...
  if (flag=='1') {
    ...
    /*****************************************************************/
    /* len2=(e1[0]-e2[0])*(e1[0]-e2[0])+(e1[1]-e2[1])*(e1[1]-e2[1]); */
    /* len6=pow((e1[0]-e2[0]),3);				     */
    /* len12=len6*len6;						     */
    /* A=0.1*pow(1.0,6);					     */
    /* B=0.1*pow(1.0,12);					     */
    /* (*energy).eata[2]=A/len6-B/len12;			     */
    /*****************************************************************/
    /* step=int((fabs(e1[0]-e2[0])*10)/(ini*10)); */
    /* (*energy).eata[2]=10.0*step;		  */
  }
  if (flag=='2') {
    (*energy).eata[1]=0.5*k1*((*p2)-e2[0])*((*p2)-e2[0])+0.5*k2*((*h2)-e2[1])*((*h2)-e2[1]);
    (*energy).eata[2]=0.5*k2*(e1[0]-e2[0])*(e1[0]-e2[0])+0.5*k2*(e1[1]-e2[1])*(e1[1]-e2[1]);
    //    (*energy).eata[2]=0.5*k2*(sin(e1[0])-sin(e2[0]))*(sin(e1[0])-sin(e2[0]))+0.5*k2*(sin(e1[1])-sin(e2[1]))*(sin(e1[1])-sin(e2[1]))+0.5*k2*(cos(e1[0])-cos(e2[0]))*(cos(e1[0])-cos(e2[0]))+0.5*k2*(cos(e1[1])-cos(e2[1]))*(cos(e1[1])-cos(e2[1]));
  }
  ...
  (*energy).w=(*energy).eata[0];
  if (flag=='2') {
    (*energy).w+=(*energy).eata[1];
    (*energy).w+=(*energy).eata[2];
  }
