...
int main(int argc, char *argv[]) {
  ...
  pi=acos(-1.0);
  if (ambcrdflag==OFF && pdbflag==OFF) {
    if (initialstep>0) {
      for (i=0;i<initialstep;++i) {
	if (flagof=='c') fprintf(outputfile,"%d ",i);
	myncL_open_inq_get_sh_MCD(inputfilename,numatom,0,1,1,&nc_id_MCD,crd_nc);
      }
    }
  }
  else if (amberncflag==ON && pdbflag==OFF) {
    if (initialstep>0) {
      for (i=0;i<initialstep;++i) {
	if (flagof=='c') fprintf(outputfile,"%d ",i);
	myncL_open_inq_get_sh_AMBER(inputfilename,numatom,0,1,1,&nc_id_MCD,crd_nc);
      }
    }
  }
  else if (ambcrdflag==ON){
    ...
    PD.PDBa=(struct PDBatom *)gcemalloc(sizeof(struct PDBatom)*numatom);
    for (i=0;i<numatom;++i) {
      PD.PDBa[i].HETEROflag=0;
      PD.PDBa[i].serial=0;
      PD.PDBa[i].ChainID=0;
      PD.PDBa[i].resSeq=0;
      for (j=0;j<3;++j) PD.PDBa[i].coord[j]=0.0;
      PD.PDBa[i].occupancy=0.0;
      PD.PDBa[i].tempfact=0.0;
    }

    inputfile=efopen(inputfilename,"r");
    readPDB(inputfile,PD,numatom);
    //    fclose(inputfile);
    for (i=0;i<numatom;++i)
      for (j=0;j<3;++j)
	crd_nc[i][j]=PD.PDBa[i].coord[j];
  }

  if (crdflag==ON) inputfile=efopen(inputfilename,"r");
  if (Drestoutflag==ON) fprintf(outputfile,"%e %d ",fc,numdihed[j]);
  for (i=initialstep;i<numstep;++i) {
    if (flagof=='c') fprintf(outputfile,"%d ",i);
    if (ambcrdflag==OFF && crdflag==OFF && amberncflag==OFF && pdbflag==OFF)
    ...
    else if (ambcrdflag==OFF  && pdbflag==OFF){
      crd=(double *)gcemalloc(sizeof(double )*numatom*3);
  ...
  if (ambcrdflag==ON || /*pdbflag==ON ||*/ crdflag==ON)
    fclose(inputfile);
  fclose(outputfile);
