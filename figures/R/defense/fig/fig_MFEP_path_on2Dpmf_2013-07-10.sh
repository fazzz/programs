...
name.out <- paste(dir,"/eps/","fig_MFEP_path_on2Dpmf_2013-07-10",sep='')
...
felwFillConMapwpath(name.pmf,name.path,label.x=label.x,label.y=label.y,level=level,title=title,kt="F/kBT",mai=c(0.5,0.5,0.1,0.0),xrange=xrange,yrange=yrange,xrange.axis=xrange.axis,yrange.axis=yrange.axis,plot.axis="yes")

par(mar=c(5,4,1,1))

felwFillConBar(name.pmf,label.x=label.x,label.y=label.y,level=level,title=title,mai=c(1.348,0.75,0.1,0.75))
