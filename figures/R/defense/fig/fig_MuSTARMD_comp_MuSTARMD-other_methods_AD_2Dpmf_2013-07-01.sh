...
name.out <- paste(dirout,"/eps/","fig_MuSTARMD_comp_MuSTARMD-other_methods_AD_2Dpmf_2013-07-01",sep='')
...
felwFillConMap(name,label.x=label.x,label.y=label.y,level=level,
              ...
              mai=c(0.0,0.8,0.5,0.0),
              xaflag="t")

felwFillConBarwmai(name,label.x=label.x,label.y=label.y,level=level,
                   mai=c(0.8,0.5,0.5,0.2))
