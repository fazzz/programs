...
name.out <- "/home/yamamori/defense/fig/fel_1"
file.name <- paste(name.out,'.eps',sep='')
postscript(file.name,width=7.0,height=7.0,horizontal=FALSE,onefile=FALSE,paper="special")
...
#                       phi=200)           # see option phi in persp
                      phi=220)           # see option phi in persp
                      {
z   <- d$z
nrz <- nrow(z)
ncz <- ncol(z)
...
persp(d,col=fcol,zlim=zlim,theta=theta,phi=phi,box="TRUE",axes="TRUE",xlab="",ylab="",zlab="")
