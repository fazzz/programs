...
dir0 <- "/home/yamamori/calspa/TACCM_CGAAREMD/FiveAtomSysd"
dirbase <- paste(dir0,"/e_CG-FG_NH_2012-05-01",sep="")
...
for ( i in 1:ntau ) {
  for ( j in 1:nmZ ) {
    for ( k in 1:nTZs ) {
      name <- paste(dirbase,"/tau=",tau[i],"/mZ=",mZ[j],"/TZ=",TZs[k],"/",pname,"/nEX=",numEX,"/pmf/pmf_TAA=",TAA,"_TCG_",TCG,"_TZ_",TZs[k],"_0.1_",KZAAo,"_",KZCGo,"_",AACG,sep="")
