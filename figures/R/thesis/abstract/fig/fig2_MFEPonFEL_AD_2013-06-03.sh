...
name.out <- paste(dir,"/eps/","fig2_MFEPonFEL_AD_2013-06-03",sep='')
   
level <- seq(0.0,${height},${width})

file.name <- paste(name.out,'.eps',sep='')
#postscript(file.name,width=2.7,height=2.4,horizontal=FALSE,onefile=FALSE,paper="special")
postscript(file.name,width=3.7,height=2.8,horizontal=FALSE,onefile=FALSE,paper="special")
