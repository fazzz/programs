...
name.out <- "${dirOUT}/eps/fig4_comp_MuSTARMD-TREMD-UmbSamp_AD_1Dpmf_2013-05-26"
...
name[2] <- paste("~/papers/CG-FG_TACCM_REMD/FEL_C7eq-C7ax_REMD.txt")
name[3] <- paste("~/papers/CG-FG_TACCM_REMD/FEL_C7eq-C7ax_MuSTARMD.txt")
name[4] <- paste("~/papers/CG-FG_TACCM_REMD/FEL_C7eq-C7ax_TAMD.txt")

cat(name)

xrange <- c(0.0,1.0,5)
xrange.axis <- c(0.2,1.0,4)

label.x<-expression(paste("along the red-line in Fig.3(f)"))
...
txt <- "(c)"
text(0.1,10,txt,cex=1.4)

axis(1,xaxp=xrange.axis,lwd=1.0,cex.axis=1.0)
mtext(label.x,side=1,line=2.0,cex=0.9)
