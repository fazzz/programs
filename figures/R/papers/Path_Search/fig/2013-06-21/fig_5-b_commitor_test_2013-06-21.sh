...
filenameMGaussian=${dir}/pmf2MGaussian_2012-10-10_bo_e_CG-FG_NH_2013-05-04/tau=${tau[1]}/mZ=${mZ[1]}/TZ=${TZs[1]}/${pname}/nEX=${numEX}/fq=${TLbase}/pmf2MGaussian_TAA=${TAA}_TCG=${TCG}_${KZAAo[$mode]}_${KZCGo[$mode]}_${MODES[$mode]}_K=${K}_wx${minx}-${maxx}_wy${miny}-${maxy}_dx=0.01_dy=0.01_2.txt
...
parafile=~/papers/Path_Search/graph/fig_6-b_commitor_test_2013-06-21.R

echo -n "name.path <- c(" > ${parafile}
for i in `seq 1 100`; do
    echo -n  "\"${filenamepathbase}_${i}_L_ave_2\", " >> ${parafile}
done
echo  "sep='')" >> ${parafile}
...
name.out <- paste(dir,"/eps/","fig_6-b_commitor_test_2013-06-21",sep='')
   
level <- seq(0.0,${height},${width})

file.name <- paste(name.out,'.eps',sep='')
postscript(file.name,width=4.2,height=3.2,horizontal=FALSE,onefile=FALSE,paper="special")
...
source("~/papers/CG-FG_TACCM_REMD/fel_wFillConMapwrangewxwy_wmpath3.R")
...
felwFillConMapwrangewxwywmpath3(name.pmf,name.path,name.path2,label.x=label.x,label.y=label.y,level=level,title=title,kt="F/kBT",mar=c(5,5,1,0),xrange=xrange,yrange=yrange,plot.axis="yes")
