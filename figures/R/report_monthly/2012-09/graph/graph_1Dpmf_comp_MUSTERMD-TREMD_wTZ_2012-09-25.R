...
for (t in angles) {
  name <- NULL
  for (c in 1:nTZs) {
    name[c] <- paste(dirbase1,"/tau=",tau[1],"/mZ=",mZ[1],"/TZ=",TZs[c],"/",pname1,"/nEX=",numEX1,"/fq=",TLbase1,"/pmf/1D/pmf_pymbar_1_TAA=",TAA,"_TCG=",TCG,"_TZ=",TZs[c],"_KZAAo=",KZAAo,"_KZCGo=",KZCGo,"_",AACG,phsiflag,"@",t,sep="")
    cat(name[c],'\n')
    sen[c] <- 1
    iro[c] <- c
  }

  name[nTZs+1] <- paste(dirbase2,"/",pname2,"/nEX=",numEX2,"/freq=",TLbase2,"ps","/pmf/1D/pmf_p1",phsiflag,"@",t,sep="")
  sen[nTZs+1] <- 1
  iro[nTZs+1] <- 2
