...
dir=~/calspa/CaM/Go-like_model/SB/
dirout=~/Report/2012-12/

parafile=~/calspa/CaM/Go-like_model/SB/para/para_e_vT_vpara_GOLMCA_JMB2001_NH_2013-01-14.sh
source ${parafile}

paraRfil=${dirout}/graph/graph_Q_single_well_CA_Go_Calmodulin_2013-01-14.R

cat <<EOF > ${paraRfil}
proname <- c( "${proname[1]}" , "${proname[2]}" )
...
cutoff <- c( "${cutoff[1]}", "${cutoff[2]}" )

ep <- c("${ep[1]}", "${ep[2]}")

name.title <- paste("${dirout}/tiff/fig_Q_single_well_CA_Go_Calmodulin_2013-01-14",sep="")
