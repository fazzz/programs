...
dirmod=~/calspa/TAMD_econ/dipep_2015-01-28_ABAMD_dubugged_mod_parm
...
direconbasemod=econ_wdi_NVE_AMBER_TermOn_tune_dihedafclust_and_min_PC6

dirOUT=~/Report/2015-02

basename=fig_econ_vdt_100ps_wdi_NVE_ABAMD_AMBER_TermOn_sander_afclust_and_minimize_all_modvs1stfixedvs2ndfixedvsfree_2015-02-02
...
for  ires in `seq 1 ${NRES}`; do
    ...
    econmod[$k]=\"${dirmod}/${RES[$ires]}/${direconbasemod}_${clustnamemod[$ires]}/${RES[$ires]}Dv_econ_300_100ps_1-10.econ.rmsd.av\"
...
cat <<EOF > ${paraRfil}
 ...
 id.ys  <- c(2, 2, 2, 2)
 ids.ys <- c(3, 3, 3, 3)
 tenshu <- c(20, 20, 20, 20)
 senshu <- c(1, 1, 1, 1)
 iro    <-c( "red", "black", "green", "blue" )
 file.names<-NULL
