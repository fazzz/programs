...
dir02 <- "~/calspa/refcalc/UmbSam/AD"
dirbase2 <- paste(dir02,"/s_UmbSam_vac_2012-11-12_",ff,sep="")
...
for (t2 in 1:numy) {
  for (t in 1:numx) {
    ...
    if ( phsiflag=="phi" ) {
      xy <- "x"
      name[1] <- paste(dirbase2,"/",pname2,"/pmf/1D/pmf_1D_UmbMD_vac_",TLns,"ns_",xy,"=",angle,".txt",sep="")
    }
    if ( phsiflag=="psi" ) {
      xy <- "y"
      name[1] <- paste(dirbase2,"/",pname2,"/pmf/1D/pmf_1D_UmbMD_vac_",TLns,"ns_",xy,"=",angle,".txt",sep="")
    ...
    for (n in 1:nparameters ) {
      m <- n+1
      name[m] <- paste(dirbase1,"/tau=",tau[n],"/mZ=",mZ[t2],"/TZ=",TZs[t],"/",pname1[n],"/nEX=",numEX1[n],"/fq=",TLbase1[n],"/pmf/1D/pmf_pymbar_1_TAA=",TAA[n],"_TCG=",TCG[n],"_TZ=",TZs[t],"_KZAAo=",KZAAo[n],"_KZCGo=",KZCGo[n],"_",AACG[n],phsiflag,"@",angle,sep="")
      cat(name[m],'\n')
      id.xs[m] <- 1
      id.ys[m] <- 2
      ids.ys[m] <- 3
