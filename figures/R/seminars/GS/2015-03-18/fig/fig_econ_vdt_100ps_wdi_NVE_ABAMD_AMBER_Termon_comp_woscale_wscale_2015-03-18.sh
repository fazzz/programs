...
parafilename=para_fig_econ_vdt_100ps_wdi_NVE_ABAMD_AMBER_Termon_comp_woscale_wscale_2015-03-18.sh
...
dir=~/calspa/TAMD_econ/dipep_2015-01-28_ABAMD_dubugged_mod_parm/

direconbase=econ_wdi_NVE_AMBER_TermOn_tune_dihedafclust_and_min_${integtype}

basename=fig_econ_vdt_100ps_wdi_NVE_ABAMD_AMBER_Termon_comp_woscale_wscale_2015-03-18
...
for  ires in `seq 1 ${NRESM}`; do
    ...
    resname[$i]=\"${RES[$ires]}\", 
    direcon[$i]=\"${dir}/${RES[$ires]}/${direconbase}_${clustname[$k]}\", 
    k=`expr ${k} + 1 `
done
