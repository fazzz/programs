program pmftwod
  ...
  double precision bin_size, maxx, minx, maxy, miny, maxpm
  ...
  do i=1,num_bin_x,1
     do j=1,num_bin_y,1
        if (hist(i,j) .ne. 0.0d0) then
           hist(i,j) = -1.0*log(hist(i,j)/bin_size/bin_size/N)
  ...
  do i=1,num_bin_x,1
     do j=1,num_bin_y,1
        print *,minx+bin_size*i-bin_size*5.0e-1, miny+bin_size*j-bin_size*5.0e-1 ,hist(i,j)
     end do
     print *,' '
  end do
