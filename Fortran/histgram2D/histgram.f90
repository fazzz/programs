program histgramtwod
  ...
  num_bin_x=int((maxx-minx)/bin_size) + 1
  num_bin_y=int((maxy-miny)/bin_size) + 1
  ...
  do i=1,N,1
     k=int((data(1, i)-minx)/bin_size)+1
     l=int((data(2, i)-miny)/bin_size)+1
     hist(k, l) = hist(k, l) + 1
  end do

  do i=1,num_bin_x,1
     do j=1,num_bin_y,1
        print *,minx+bin_size*i-bin_size*5.0e-1, miny+bin_size*j-bin_size*5.0e-1 ,hist(i,j)/bin_size/bin_size/N
     end do
     print *,' '
  end do
