...
!    write(unitnum,'("MODEL")')
    do i = 1,N
       ...
       write(unitnum,'(A6,I5,1X,A4,A1,A3,1X,A1,I4,A1,3X,3F8.3)'),&
            nameRecord,PA(i)%serial,PA(i)%atm_name,alte_l_i,PA(i)%res_name,&
            chain_i,mod(PA(i)%res_seq_num,9999),code_for_insertion_res,&
            PA(i)%x,PA(i)%y,PA(i)%z
...
!    write(unitnum,'("ENDML")')
  ...
  subroutine read_PDB (unitnum, PA, N)
    ...
    do
       ...
       read(unitnum,'(A6,I5,1X,A4,A1,A3,1X,A1,I4,A1,3X,3F8.3,2F6.2,10X,A2,A2)',end=999),&
            nameRecord,&
            PA(i)%serial,PA(i)%atm_name,alte_l_i,PA(i)%res_name,chain_i,&
            PA(i)%res_seq_num,code_for_insertion_res,PA(i)%x,PA(i)%y,PA(i)%z,&
