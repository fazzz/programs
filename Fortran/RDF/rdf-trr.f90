...
program rdf
    ...
    do while ( trr % STAT == 0 )
        ng = ng + 1
        posA = trr % pos(:, 1:nposA)       !get the position of atoms of mol A
        posB = trr % pos(:, nposA+1:npos)  !get the position of atoms of mol B
...
contains
  subroutine usage(programname)
    ...
    print *,"Usage: ",programname," [ inputfilename (trr file) ] outputfilename itpfilename2"
    stop
