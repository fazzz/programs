...
use bestfit
        ...
        call jacobi( refslt_natom, varm, ev, vec )
...
subroutine jacobi( refslt_natom, varm, ev, vec )
        double precision, intent(inout) :: varm(refslt_natom, refslt_natom, 3)
        double precision, allocatable, intent(out) :: ev(:,:), vec(:,:,:)
        integer :: i, j, k, l, lm, refslt_natom
        double precision :: s, c, aoff, eps, t, aa, aj, ak, vj, vk, vmax, bb
        allocate( ev(refslt_natom, 3), vec(refslt_natom, refslt_natom, 3) )
        ...
        aoff = 0.0
        do i = 1, 3
           do j = 1, refslt_natom - 1
              do k = j+1, refslt_natom
                 aoff = aoff + varm(j,k,i) * varm(j,k,i)
              enddo
           enddo
        eps = aoff * 2.0
           do j = 1, refslt_natom
              eps = eps + varm(j,j,i) * varm(j,j,i)
           enddo
        eps = eps * 0.00000000005
           do while ( aoff > eps )
             do j = 1, refslt_natom - 1
               do k = j+1, refslt_natom
                  if (varm(j,k,i) == 0.0) then
                        t = 0.0
                  else
                        aa = (varm(k,k,i) - varm(j,j,i)) / (2.0*varm(j,k,i))
                  endif
                  if (aa >= 0) then
                  ...
                  c = 1.0 / sqrt(1.0+t*t)
                  s = t*c
                  varm(j,j,i) = varm(j,j,i) - t*varm(j,k,i)
                  varm(k,k,i) = varm(k,k,i) + t*varm(j,k,i)
                  varm(j,k,i) = 0.0
                  do l = 1, j-1
                        aj = varm(l,j,i)
                        ak = varm(l,k,i)
                        varm(l,j,i) = aj*c - ak*s
                        varm(l,k,i) = aj*s + ak*c
                  enddo
                  do l = j+1, k-1
                        aj = varm(j,l,i)
                        ak = varm(l,k,i)
                        varm(j,l,i) = aj*c - ak*s
                        varm(l,k,i) = aj*s + ak*c
                  enddo
                  do l = k+1, refslt_natom
                        aj = varm(j,l,i)
                        ak = varm(k,l,i)
                        varm(j,l,i) = aj*c - ak*s
                        varm(k,l,i) = aj*s + ak*c
                  enddo
                  do l = 1, refslt_natom
                        vj = vec(l,j,i)
                        vk = vec(l,k,i)
