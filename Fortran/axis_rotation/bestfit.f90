...
contains
  ...
  subroutine prod(q1, q2, p)
    implicit none
    double precision, intent(in)  :: q1(0:3), q2(0:3)
  ...
  subroutine rotate(q, r, res)
    implicit none
    double precision, intent(in)  :: q(1:3), r(0:3)
    double precision, intent(out) :: res(1:3)
    double precision :: t(0:3), cj(0:3)
    double precision :: q2(0:3), res2(0:3)
    q2(0) = 0.
    q2(1:3) = q(:)
    call conjugate(r, cj)
    call prod(r, q2,  t)
  ...
  subroutine rotate_inplace(n, coord, rot_quaternion)
    implicit none
    integer, intent(in) :: n
    double precision, intent(in) :: rot_quaternion(0:3)
    double precision, intent(inout) :: coord(3, n)
    integer :: i
    double precision :: temp(3)
    do i = 1, n
...
contains  
  ...
  subroutine find_rotation_quaternion(n, refPt, movedPt, masses, rotation)
    ...
    external dsyev
    integer :: n, info
    double precision, intent(in) :: refPt(3, n), movedPt(3, n), masses(n)
    double precision, intent(out) :: rotation(0:3)
    double precision :: inner_prod(3, 3)
    double precision :: matmax(4, 4)
    double precision :: eigenvalue(4)
...
!    use mpiproc, only: halt_with_error
    implicit none
    integer, intent(in) :: n
    double precision, intent(in) :: points(3, n), masses(n)
    double precision, intent(out) :: center(3)
    double precision :: sumOfMasses
    integer :: i
    
    sumOfMasses = sum(masses)
    if(sumOfMasses == 0) write(*,*),"error" !call halt_with_error('bst_zrw')
    do i = 1, 3
  ...
  subroutine com_shift(n, points, masses, center)
    implicit none
    integer, intent(in) :: n
    double precision, intent(inout) :: points(3, n)
  ...
  subroutine fit(n, refcoord, coord, masses, outcoord)
    implicit none
    integer, intent(in) :: n
    double precision, intent(in) :: refcoord(3, n), coord(3, n), masses(n)
    double precision, intent(out) :: outcoord(3, n)
  ...
  subroutine fit_a_rotate_b(na, refa, a, massa, nb, b, bout)
    ...
    double precision, intent(in) :: refa(3, na), a(3, na), massa(na)
    double precision, intent(in) :: b(3, nb)
    double precision, intent(out) :: bout(3, nb)
    
    double precision :: workref(3, na), work(3, na)
    double precision :: com_refa(3), com_a(3)
    double precision :: bcrd(3), bcrdr(3)
    double precision :: rotation(0:3)
    integer :: i
  ...
  double precision function rmsd_bestfit(n, refcoord, coord, masses)
    implicit none
    integer, intent(in) :: n
    double precision, intent(in) :: refcoord(3, n), coord(3, n), masses(n)
    double precision :: fitted_coord(3, n)
  ...
  double precision function rmsd_nofit(n, crdA, crdB, masses)
!    use mpiproc, only: halt_with_error
    implicit none
    integer, intent(in) :: n
    double precision, intent(in) :: crdA(3, n), crdB(3, n), masses(n)
    integer :: i
    double precision :: sumOfMasses, disp, dx(3)
    sumOfMasses = sum(masses)
    if(sumOfMasses == 0) write(*,*),"error" !call halt_with_error('bst_zrw')
    disp = 0.0
    do i = 1, n
