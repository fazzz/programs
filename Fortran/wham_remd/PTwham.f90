! This program calculates 2D-Free Energy Landscape (1D or 2D)
! from TREMD simulation data. (2018-02-22)

program wham
  implicit none

  real :: b, tol ! the inverse temperature and tolerance of iteration.
  real, allocatable dimension(:) :: bs ! the inverse temperatures for each simulation.
  real :: min(2),max(2)
  real :: wid_bin(2), dim,  C, d, mind, dat(2)
  real, allocatable :: ef(:), efo(:), ew(:,:,:), bV(:), z(:,:,:), z0(:,:), rouU(:,:), rouB(:,:,:), ewps(:), phi(:), psi(:)

  integer :: Ndim ! # of dimension of FEL.
...
999 close(17)
  ...
  subroutine usage(programname)
    ...
    print *,"Usage: ",programname,"inputfile1(beta) inputfile2(Ui,l) outputfile1(fk) outputfile2(fel)"
    stop
