...
!  character,allocatable,dimension(:,:) :: atype, res, atom
  ...
  character(50) ndx_reffilename, ndx_infilename
!  character(50) top_filename,
  character(50)  pdb_reffilename, pdb_infilename, pdb_outfilename
...
!  do i = 1,Nndx_ref,1
!     write(*,'(I4,1X)', advance="NO"), atom_list_ref(i)
...
!  allocate(outcoord(3,NHA))
  ...
  do i=1,Nndx_in,1 
     j=atom_list_ref(i)
     refcoord(1,i)=PAref(j)%x
     refcoord(2,i)=PAref(j)%y
     refcoord(3,i)=PAref(j)%z     
     !     write(*,*) j, PAref(j)%atm_name(2:2)
     atm_name_HEAD = PA(j)%atm_name(2:2)
...
!  do i=1,Nndx_in,1 
!     write(*,'(3F8.3)',advance="NO"), massbsf(i)
!     if (mod(i,10) .eq. 0) then
!        write(*,"()")
!     end if
!  end do
...
!    print *,"Usage: ",programname,"topfilename PDB1(reffile) PDB2(input) PDB3(output)"
    print *,"Usage: ",programname,"topfilename PDB1(reffile) PDB2(input) NDX1(reffilename) NDX2(input)"
    stop
