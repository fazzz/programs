...
program bestfit_2pdb
  ...
  integer N, NHA, Nref, NHAref ! num of atoms
...
!  real, allocatable, dimension(:) :: massHA
  ...
  double precision, allocatable, dimension(:,:) :: refcoord, coord, outcoord
  double precision, allocatable, dimension(:) :: massHA
...
!  call fit(NHA, refcoord, coord, massHA, outcoord)
  call fit(N, refcoord, coord, massHA, outcoord)

  rmsd = 0.0d0
  do i=1,N,1
     len = 0.0d0
     do j=1,3,1
        len = len + (refcoord(j,i) - outcoord(j,i))**2
     end do
     rmsd = rmsd + len
  end do
  rmsd = rmsd / N
